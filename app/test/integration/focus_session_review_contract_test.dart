...
void main() {
  ...
  // ---------------------------------------------------------------------------
  // (g) Off-Plan Dispositions have a durable home (issue #418, ADR-0015)
  //
  ...
  group('Off-Plan Dispositions persist durably (issue #418, ADR-0015)', () {
    late GtdDatabase db;
    ...
    test(
        'storing an off-Plan disposition does NOT add a focus_session_tasks '
        'row — the Plan stays fixed (re-assert (f) for the write path)',
        () async {
      await _insertTodo(db, id: 'X', title: 'X'); // Plan member
      await _insertTodo(db, id: 'Z', title: 'Z'); // off-Plan
