...
void main() {
  ...
  group('TodoDao — processInboxItem (legacy in-place promote)', () {
    late GtdDatabase db;

    setUp(() => db = _openInMemory());
    tearDown(() async => db.close());

    test('flips clarified and stamps lastClarifiedAt', () async {
      await _insertTodo(db, id: 'x', title: 'Process me', clarified: false);

      final before =
          await (db.select(db.todos)..where((t) => t.id.equals('x')))
              .getSingle();
      expect(before.lastClarifiedAt, isNull);

      expect(await db.todoDao.processInboxItem('x'), 1);
    ...
    test('only an unclarified row is touched, so re-processing is a no-op',
        () async {
      await _insertTodo(db, id: 'x', title: 'Process me', clarified: false);

      expect(await db.todoDao.processInboxItem('x'), 1);
      // Already clarified: guard returns 0 so callers can skip double-processing.
      expect(await db.todoDao.processInboxItem('x'), 0);
    });

    test('passes intent and dueDate through', () async {
      await _insertTodo(db, id: 'x', title: 'Process me', clarified: false);
      final due = DateTime.utc(2026, 8, 1);

      await db.todoDao.processInboxItem('x', intent: 'maybe', dueDate: due);
