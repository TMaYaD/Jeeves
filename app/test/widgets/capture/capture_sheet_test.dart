...
/// A launcher scaffold whose FAB opens the real capture sheet — exercises the
/// production `CaptureFab` → `showCaptureSheet` path, including dismissal.
Widget _wrapFab(GtdDatabase db) => ProviderScope(
      overrides: [databaseProvider.overrideWithValue(db)],
      child: const MaterialApp(
        home: Scaffold(
...
void main() {
  ...
  testWidgets('opening then dismissing the sheet creates no Capture',
    ...
    await tester.pumpWidget(_wrapFab(db));
    await tester.pump();

    await tester.tap(find.byKey(const Key('capture_fab')));
    await tester.pump();
  ...
  testWidgets('CaptureFab is a genuine circle in the capture blue',
      (tester) async {
    final db = GtdDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    ...
    expect(fab.backgroundColor, const Color(0xFF2563EB));
  });
}
