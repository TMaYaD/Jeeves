...
- **`watchNext` lets actionless PersonBlocked Outcomes onto Next** — they should be Waiting-For-only. The Next List rule is `intent='next' ∧ Completion=null ∧ (next_action_text IS NOT NULL ∨ no PersonBlocker)`: the only excluded combination is *actionless + PersonBlocker* (the pure-wait case). `TodoDao.watchNext` currently filters on `intent='next' ∧ clarified ∧ !done` only, so a pure-wait Outcome leaks onto Next. An *actionable* PersonBlocked Outcome correctly appears on both Next and Waiting For (overlap by design). Tracked in #361 — the corrected filter, four-quadrant tests, and tightened wording here and in `docs/ARCHITECTURE.md` land there.
...
- **Time-based Triggers re-evaluate at their boundaries; the content-state dismiss edge does not yet.** Dismiss is scoped to the current firing (persisted in the Nudge module's `NudgeState`); Nudge precedence is applied uniformly across every surface — Banner *and* Notification — by the Nudge queue's priority ordering. The *time-based* Triggers are reactive: they flip when their wall-clock boundary passes, off the shared `clockProvider`. The Nudge module's `nudgeBoundaryTickProvider` schedules a single timer to the next boundary (day rollover, today's Evening Shutdown anchor) and re-evaluates the Triggers there; the Weekly Review due predicate (`periodicReviewIsDueProvider`) schedules its *own* timer to the cadence-due instant (`last + 7d`). Either way a Nudge surfaces at its boundary rather than waiting for an unrelated rebuild. What remains is the *content-state* Trigger's per-firing dismiss edge: its `firingSince` is still a stable per-day stamp, so a content-state dismiss releases at the day boundary rather than on the next genuine false→true content edge. Persisting and tracking that content firing edge is tracked in #363.
