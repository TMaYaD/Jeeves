...
- The schema now splits **Capture** from **Outcome**: dedicated `captures`, `capture_outcomes` (the M:N provenance join), and `capture_tags` (tag hints) tables exist and replicate (ADR-0006, issue #184; Alembic 0026 non-destructively moves the old `clarified = false` rows out of `todos`). The remaining gap is the app-layer cutover: the Inbox and clarify UI still read and write the conflated `todos.clarified` flag, and `ClarificationService` still writes the conflated row, until the clarify flows are rewired onto the new tables. Until then the two representations coexist.
...
- **`clarified` (boolean)** on the Todo row is an artifact of the Capture/Outcome conflation. It is still live today: the current Inbox and clarify flows read and write it (see the split-gap bullet above), and old clients during the staged rollout keep writing `clarified = false` rows. It becomes unread only once the app-layer cutover onto `captures` lands; the column is then retained (still unread) through one release train and dropped in a follow-up migration that first sweeps any straggler `clarified = false` rows. **`last_clarified_at`** is a distinct concept — staleness of the *current* clarification — and survives the split.
