...
- **The op log is the whole client sync stack.** The Workspace security cluster is what runs in the app: a Device enrols on the passphrase alone, pins Root, founds or joins each of its User's two implicit Workspaces with a Root-signed control op, holds an explicit Root-signed **owner** Grant, and verifies every author's chain and every authority on the way in. Enrolment is also what *starts* syncing — the **Initial Upload** authors the store the user already has, and from then on every domain write is an op (issues #591, #595; ADR-0034, ADR-0035). Implemented: **Workspace** as a genesis-signed partition, **Member**, **Root**, **Recovery**, **Author**, **Integrity Alarm**, and **Grant** with its four roles.

- **KeyWrap, epochs and the `aead_v1` suite are half-built (#554).** The *protocol* is complete and pinned by golden vectors on both sides: the encrypted envelope, both KeyWrap flavours, the digest, and the **Rotate** control op (the sealed box is hand-rolled — ADR-0037). The *server* is complete: it stores wraps it cannot open, serves them per Member, serves the escrow wraps, materialises Rotate into a per-epoch index, and refuses content more than one epoch stale. What is **not** built is the client half — no Device holds a Workspace key, mints a wrap, runs the rotation ceremony, or emits an `aead_v1` op. So in the app as it stands every Grant is still implicitly epoch-0, the *orphaned grant* predicate still never fires, and the persisted epoch floor is still raise-only machinery with nothing raising it. Encryption turning on is a per-Workspace ceremony an owner runs, never a deploy, which is why the server can ship this far ahead of the client with nothing changing for a `plaintext_v1` device.
