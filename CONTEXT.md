...
**The Action rows are the only grain.** The `actions` table is where an Action's existence, text, role and effort metadata live, and it is the only thing the app reads or writes to answer "what is this Outcome's next move?". The legacy one-field cursor, `todos.next_action_text`, was retired by abandonment and then **dropped from the schema**: there is no Outcome column holding a next-action phrase, and no invariant ties one to the Action rows. An Outcome reduced in from a client that predates the Actions table renders Actionless until it is re-clarified. There is no startup sweep: the multi-`current` repair it used to perform lives where it always belonged, in the writers — every `ActionDao` primitive that touches the current Action converges an accidental multi-`current` set onto the same deterministic winner first, so the set is repaired as it is encountered rather than once per launch.
...
visible =
  ...
  ∧ (no Ceremony performance of this Ritual is currently in progress)   ← in-progress hygiene
...
- The **in-progress hygiene rule** is centralised at the Nudge level: while any Ceremony performance of this Ritual is in progress, no Nudge of this Ritual is visible regardless of Trigger state. This is the one non-content rule the Nudge model carries.
