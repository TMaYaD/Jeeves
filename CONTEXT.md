...
- **`clarified` (boolean)** on the Todo row is a leftover of the Capture/Outcome conflation. The live Inbox is `captures.clarified_at IS NULL`, so the legacy todos-based inbox read surface has been removed: the old `InboxDao` (its `watchInbox` / `watchHasTodos` / `insertTodo` / `deleteTodo`) is gone, and its one still-referenced write, the in-place 1:1 promote, now lives on `TodoDao.processInboxItem`. That promote is the only code left that *reads* `clarified` (it flips `clarified = false → true`), and it is itself production-dead — reached only through `ClarificationService.promoteCaptureToOutcome`, which has no production caller and is pinned by tests alone. `clarified` is still *written* by `TodoDao.applyRouting` / `insertOutcome` when an Outcome is created, and old clients during the staged rollout keep writing `clarified = false` rows that the release-train migration sweeps. The column is retained (effectively unread in production) through one release train and dropped in a follow-up migration. **`last_clarified_at`** is a distinct concept — staleness of the *current* clarification — and survives the split.
