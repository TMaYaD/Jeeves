...
- The **Action** entity now exists as first-class rows (`actions` table, ADR-0001) and is *written* on every next-action write path (issue #472 story 2): `ActionDao` maintains the Outcome's `current` Action — create, in-place text edit, explicit supersession without linkage metadata (ADR-0018), and clear — and the legacy cursor-field writes dual-write it. But *reads* still come from the Outcome cursor-fields (`next_action_text`, `energy_level`, `time_estimate`, `time_spent_minutes`, `last_next_action_completion_at`), which remain authoritative until the read-cutover (story 3); a startup sweep repairs any cursor→Action drift. The remaining reconciliation — moving reads to the Action row, completion semantics, planned-Action promotion/demotion, live metadata wiring, and the naming rename — lands over stories 3–9. Until then the cursor-fields and the Action rows coexist, kept in agreement by the dual-write and the sweep.
