...
- The codebase uses "Todo" (DB table, Drift class) and "Task" (docs, foreign-key columns like `task_id`) for what the model calls an **Outcome**. Dart identifiers still say `nextActionText` where they mean an Action's text — a parameter name on the legacy one-field surfaces, not a storage reference (the column of that name is retired, ADR-0022); renaming them to `actionText` is pending mechanical work. The proposal at `docs/proposals/blockers-and-contexts.md` uses "Task" and "NextAction" throughout; that document predates these renames.
