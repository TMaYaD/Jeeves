...
**The Action rows are the only grain.** The `actions` table is where an Action's existence, text, role and effort metadata live, and it is the only thing the app reads or writes to answer "what is this Outcome's next move?". The legacy one-field cursor, `todos.next_action_text`, was retired by abandonment (ADR-0022) and then **dropped from the schema** (ADR-0024): there is no Outcome column holding a next-action phrase, and no invariant ties one to the Action rows. An Outcome synced in from a client that predates the Actions table renders Actionless until it is re-clarified. The startup sweep reads no Outcome column at all: it never adopts an Action from one and never retires one under one. Its one pass, `convergeMultiCurrentActions`, only repairs an accidental multi-`current` set by retiring the losers to a single winner under the same deterministic rule the writers use — it reads `actions` and nothing else.
...
_Avoid_: Reseed (the name of the cutover-scoped *verification* tooling only), Migration (a ceremony this deliberately is not), Backfill (implies filling a gap rather than carrying a store)

#### Relationships

- **Sync types do not appear in GTD Core / Engagement / Ceremony Framework function signatures.** The dependency direction is strictly one-way: those three contexts speak Drift/in-memory entities; Sync mechanically replicates those entities without adding semantics. The seam is the capture binding — a DAO write path describes *what changed* and never who is listening, which is what lets an un-enrolled device run the same write paths and author nothing. Any change that would add a Sync-tier type (Bucket, Sync Token, BackendConnector reference, a `SyncClient`) to a method signature in another context is the warning sign.
...
- **The op log is the live system; the PowerSync entries below describe a disconnected path.** The Workspace security cluster is what runs in the app: a Device enrols on the passphrase alone, pins Root, founds or joins each of its User's two implicit Workspaces with a Root-signed control op, holds an explicit Root-signed **owner** Grant, and verifies every author's chain and every authority on the way in. Enrolment is also what *starts* syncing — the **Initial Upload** authors the store the user already has, and from then on every domain write is an op (issue #591, ADR-0034). Implemented: **Workspace** as a genesis-signed partition, **Member**, **Root**, **Recovery**, **Author**, **Integrity Alarm**, and **Grant** with its four roles. The **Sync Shape**, **Bucket**, **BackendConnector**, **Dead Letter** and **Sync Token** entries below still describe real code and real tables, but nothing exercises them: the engine no longer connects, so no bucket is replicated, no CRUD entry is uploaded and no Dead Letter can be recorded. Their retirement, entry by entry, rides #556 with the engine. Still to come: **KeyWrap** and key epochs (#554), so every Grant is implicitly epoch-0, the *orphaned grant* predicate is defined against epoch 0 and never fires, and the persisted epoch floor is raise-only machinery with nothing yet to raise it.
...
_Avoid_: Dead Letter (the PowerSync-era term for an unrelated mechanism), Rejected op (a rejection is the server's answer to an append, not the receiving Device's record), Discarded (nothing is discarded)
...
**PowerSync** *(the retired Sync engine, still on device as a store)*:
The replication engine Jeeves used to sync with — a self-hosted `journeyapps/powersync-service` instance over a mirrored PostgreSQL schema. **Disconnected since #591**: the app never calls `connect()`, so nothing replicates through it. What remains is the on-device SQLite store the domain read model lives in, plus the server-side mirrored tables and REST routes; all of it goes with #556. The engine swap Sync's discipline rule was protecting against is the one that happened.
_Avoid_: "the Sync engine" unqualified (the op log is the Sync engine now)

**Sync Shape** *(one of the twelve replicated data subsets — PowerSync-era, disconnected)*:
...
**Bucket** *(the runtime replication unit — PowerSync-era, disconnected)*:
A single user's instance of a Sync Shape — the replication stream PowerSync materialised while it was connected. None is materialised now. One Bucket exists per `(user, Sync Shape)` pair. The user never sees a Bucket; the term exists for code and operational debugging.
_Avoid_: Sync stream (vaguer), Channel (overloaded), Shard

**BackendConnector** *(the local-to-backend upload path — PowerSync-era, no caller)*:
The interface (`JeevesBackendConnector` in code) PowerSync used to upload local writes to the Jeeves backend's REST API. Local writes still *queue* in `ps_crud`, because the tables are PowerSync-managed, but nothing drains the queue: the connector has no caller (ADR-0034), and turning the enqueueing off needs the store swap #556 owns. The Sync engine reads from the backend's PostgreSQL directly; only writes flow through the connector.
_Avoid_: Sync connector (acceptable but BackendConnector is the canonical class name), Upload pipe

**Dead Letter** *(a recorded non-retryable upload failure — PowerSync-era, none can occur)*:
No new one can be recorded, since nothing uploads through the BackendConnector; the table and any rows a pre-flip device holds ride #556. A diagnostic record of a queued local write the BackendConnector could not upload for a non-retryable reason (per-status policy in `docs/ARCHITECTURE.md § Upload-error policy`), persisted in the local-only `sync_dead_letters` table with the operation, table, payload, status, and response body. Developer telemetry, not a user-facing retry queue: the failure surfaces through the sync indicator, and resolution means fixing the root cause so the table trends toward empty.
_Avoid_: Failed upload queue (implies replay), Sync error log (vaguer)

**Sync Token** *(PowerSync-era, unfetched)*:
The short-lived JWT issued by the backend's `GET /powersync/credentials` endpoint and consumed by PowerSync for authentication. The endpoint is still served; no client fetches it. Distinct from the **member-scoped token** the op log's proof-of-possession exchange mints, which is what a Device actually syncs with. The backend signs it with `SECRET_KEY`; PowerSync validates with the same key. Distinct from the user's auth tokens (access / refresh JWTs from the auth provider), though the v1 implementation shares the signing key.
