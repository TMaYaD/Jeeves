...
- A Capture carries only title and notes. Energy level, time estimate and due date are **Outcome** attributes with no column on a Capture, so a clarify card holds them as draft state and they are written onto the Outcome at clarification. Tag *hints* (`capture_tags`) are the exception: they persist on the Capture and seed the new Outcome's tags, so they survive Skip and Back.
