...
**Title-as-action fallback.** When a surface routes an Outcome (or the Outcome a Capture clarifies into) to **Next** or **Waiting For** and the user supplied no next-action phrase, the Outcome's own **title** stands in as the phrase — written only when the Outcome is **Actionless**, so a deliberate existing Action is never clobbered, and skipped only when the title itself is blank. For a simple Outcome the title genuinely names the physical next action ("Call the dentist"), and a **Project** — one requiring more than one Action over its lifetime — is the case that earns a distinct phrase, not the default. Leaving no phrase therefore always resolves the item and moves the user on; only an explicit *cancel* leaves it unresolved. Clearing an existing phrase is not how an Action is retired — **Abandon** is.
