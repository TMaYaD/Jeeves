...
- The **Action** entity now exists as first-class rows (`actions` table, ADR-0001) and is *written* on every next-action write path (issue #472 story 2): `ActionDao` maintains the Outcome's `current` Action — create, in-place text edit, explicit supersession without linkage metadata (ADR-0018), and clear — and the legacy cursor-field writes dual-write it. *Reads* of the current Action — whether an Outcome has one, and its text — now come from `actions` too (issue #473 story 3), so `next_action_text` is write-only compatibility until story 9 removes it. The Outcome's *metadata* and *timestamp* cursor-fields (`energy_level`, `time_estimate`, `time_spent_minutes`, `last_next_action_completion_at`) are still the read source, until stories 4/6/7 move each in turn; a startup sweep repairs any cursor→Action drift. The remaining reconciliation — completion semantics, planned-Action promotion/demotion, live metadata wiring, and the naming rename — lands over stories 4–9. Until then the cursor-fields and the Action rows coexist, kept in agreement by the dual-write and the sweep.
