...
- **A User participates in one or more Workspaces — exactly one today.** Participation is the set of grants held by that User's keyholders, not an ownership column on the Workspace. An **Area** belongs to exactly one Workspace; a Workspace holds many. Everything else a Workspace contains (Captures, Outcomes, Actions, Tags, FocusSessions, TimeLogs) reaches its Workspace through the Area or, being Area-less, sits directly in it.
