...
- **Area exclusivity is conceptual-only so far.** The model says an Outcome belongs to at most one Area (ADR-0025); storage still permits many, since Areas are `Tag(type='area')` rows linked through `todo_tags`, which carries no uniqueness constraint. Reconciliation needs the constraint plus a migration over Outcomes that currently hold several — resolved by user choice, with the surplus memberships converted to **Labels** rather than dropped, so no categorisation is lost.
