...
- The app now runs on the split model end to end: the Inbox reads `captures WHERE clarified_at IS NULL`, quick-add writes a Capture, and every clarify surface goes through `ClarificationService.clarifyCaptureToOutcome` / `discardCapture` — creating an Outcome, writing the `capture_outcomes` provenance link, and stamping `clarified_at` (ADR-0006, issue #184). The **mode** is now a real synced preference (`clarify_mode` in `user_preferences`, default `oneToOne`, exposed at Settings → Clarify), but only the 1-1 side of it is implemented: every clarify surface still stamps at the first Outcome link regardless of the stored mode. Selecting **n-m** therefore changes nothing yet — the unified capture-to-Outcome field and the completing footer that give the mode its behaviour land with the split/merge clarify UI. The preference deliberately leads the implementation so the choice replicates and settles before the surfaces that read it exist.
