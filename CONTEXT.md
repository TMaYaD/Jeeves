...
**Clarification never rewrites a Capture** (ADR-0023). The clarify surface seeds its fields from the fragment and feeds the Outcome draft; the Capture keeps exactly what was captured, so what the system holds as "what I captured" stays the raw record rather than the interpretation of it. The visible consequence is that the Inbox shows what you captured, not your working copy: correct a typo mid-clarify and Skip, and the Inbox still shows the typo while the correction rides the draft onto the Outcome when you route. Because nothing persists the in-progress text, it is retained in memory for the duration of a Ceremony performance so Back and Skip do not lose it.
...
**Copy ↔ model.** The user-facing verb for supersession is **Abandon** ("Abandoned" in the past tense, on a history row); the model term stays `superseded`. Completion reads **Done** ↔ `role='done'`. Abandoning is a distinct act from **Remove**, which hard-deletes an unengaged `planned` Action: an Abandoned Action is retired into history and stays there, a Removed one never happened. Neither the history rows nor anything else can un-abandon, re-promote, or edit a terminated Action — it is a record.

The history chain is honest about *what* terminated, not about *who* terminated it. A row retired by the startup reconciliation sweep (a phantom current Action repaired away) is stored as `superseded` and therefore reads as "Abandoned", indistinguishable from one the user abandoned deliberately. That is accepted: adding a marker column would amend ADR-0018's no-linkage-metadata decision to disambiguate a case that only arises from repair.

**The Action rows are the only grain.** The `actions` table is where an Action's existence, text, role and effort metadata live, and it is the only thing the app reads or writes to answer "what is this Outcome's next move?". The legacy one-field cursor, `todos.next_action_text`, was retired by abandonment (ADR-0022) and then **dropped from the schema** (ADR-0024): there is no Outcome column holding a next-action phrase, and no invariant ties one to the Action rows. An Outcome reduced in from a client that predates the Actions table renders Actionless until it is re-clarified. There is no startup sweep: the multi-`current` repair it used to perform lives where it always belonged, in the writers — every `ActionDao` primitive that touches the current Action converges an accidental multi-`current` set onto the same deterministic winner first, so the set is repaired as it is encountered rather than once per launch.

**Title-as-action fallback.** When a surface routes an Outcome (or the Outcome a Capture clarifies into) to **Next** or **Waiting For** and the user supplied no next-action phrase, the Outcome's own **title** stands in as the phrase — written only when the Outcome is **Actionless**, so a deliberate existing Action is never clobbered, and skipped only when the title itself is blank. For a simple Outcome the title genuinely names the physical next action ("Call the dentist"), and a **Project** — one requiring more than one Action over its lifetime — is the case that earns a distinct phrase, not the default. Leaving no phrase therefore resolves the item and moves the user on wherever a title exists to stand in. Two things leave it unresolved instead: an explicit *cancel*, and the degenerate case of a **blank title**, where nothing is left to stand in as the Action. Clearing an existing phrase is not how an Action is retired — **Abandon** is.

Energy level and time estimate are Action-grain for the same reason: they describe the *action of doing*, so a replacement Action never inherits the effort estimate of the one it replaced. Unlike the cursor these are **not** retired — the Outcome columns remain the draft store while an Outcome is Actionless, and reads resolve to the current Action's value with the Outcome column as a per-field fallback. A *planned* Action carries its own effort from the moment it is written down, and carries it through promotion: the user estimating "that will be a big one" about a step they have not started is exactly the externalised thinking the plan is for.
...
**Area** *(of Focus)*:
A domain of recurring responsibility — David Allen's GTD horizon-2 concept. Examples: "Health", "Finance", "Family", "Open-source maintenance." An Area is a permanent (or long-lived) categorisation, and it is **exclusive**: an Outcome belongs to at most one Area. Exclusivity is what makes an Area-by-Area pass non-overlapping — no Outcome surfaces under two Areas, so the Areas plus the Area-less remainder cover everything exactly once. Cross-cutting grouping is **Label**'s job; gating an Action on where you physically are is **Context**'s job. Allen specifies the concept but no project↔area membership at all — his Projects list is flat, and Areas are a list reviewed for neglect rather than a taxonomy projects are filed into — so the cardinality here is a Jeeves choice, not GTD canon (ADR-0025).
...
- The app now runs on the split model end to end: the Inbox reads `captures WHERE clarified_at IS NULL`, quick-add writes a Capture, and every clarify surface goes through `ClarificationService` — `clarifyCaptureToOutcome` creates an Outcome, writes the `capture_outcomes` provenance link, and stamps `clarified_at`; `discardCapture` is the zero-Outcome verdict and only stamps (ADR-0006, issue #184). The mode is a real, cross-device preference (`clarify_mode`, default `oneToOne`) with a Settings toggle, and both clarify surfaces are now driven end to end (issue #434): under `nToM` the clarify body renders `CaptureOutcomesSection`, which carves and merges Outcomes one at a time and closes with a single verdict slot. **1-1 mode** is that same surface with the list empty and the form auto-opened, so the storage model and the UI now agree.
- The **Action**-entity reconciliation is complete: the model and the implementation now agree, and § Action describes the live system rather than an aspiration. The one piece still outstanding is cosmetic — the identifier rename below.
- The **title-as-action fallback** is stated for both **Next** and **Waiting For**; the implementation covers Waiting For only on the clarify surfaces. A Capture routed to Waiting For carries `ClarifyDraft.assemble`'s title mirror into the Outcome it mints, and `ClarifyCard` mirrors the title on its own Next / Waiting For routes — but the shared `ProcessToHandlers` Waiting For button, which the review steps use against an *existing* Outcome, routes intent-only and applies no fallback. The model is right and the implementation lags: closing the gap is a behaviour change on the review surfaces, not a rewording.
- **`clarified` (boolean)** on the Todo row is a leftover of the Capture/Outcome conflation. The live Inbox is `captures.clarified_at IS NULL`, so the legacy todos-based inbox surface has been removed entirely: the old `InboxDao` (its `watchInbox` / `watchHasTodos` / `insertTodo` / `deleteTodo`) and the in-place 1:1 promote (`ClarificationService.promoteCaptureToOutcome` → `TodoDao.processInboxItem`) are both gone, so **Inbox membership** no longer derives from `clarified`. The column itself is not dead: the GTD list watchers still filter `clarified = 1` as a real-Outcome guard (`watchNext` / `watchMaybe` via `_watchByIntentAndTags`, `watchPersonTagged`, `watchPersonTaggedGrouped`, `getNeedsReview`, `getNextExcludingPersonTagged`), and it is still *written* by `TodoDao.applyRouting` / `insertOutcome` when an Outcome is created, while old clients during the staged rollout keep writing `clarified = false` rows that the release-train migration sweeps. Retiring the column therefore means reworking those watcher predicates first — it is not a drop-in follow-up migration. **`last_clarified_at`** is a distinct concept — staleness of the *current* clarification — and survives the split.
- A Capture carries only title and notes, and a clarify surface writes **neither** (ADR-0023). Due date is an **Outcome** attribute with no column on a Capture, so a clarify card holds it as draft state and it is written onto the Outcome at clarification. Energy level and time estimate are **Action**-grain (issue #477 story 7): the clarify card still holds them as draft and they land on the Outcome columns at clarification, but they seed the birth Action the moment a current Action is created — so on an Actionless Outcome they stay Outcome-level draft until the next Action is created, and thereafter reads resolve them from the current Action (with the Outcome column as fallback). Tag *hints* (`capture_tags`) are the exception: they alone persist on the Capture, and they seed the new Outcome's tags. Everything else the user types is held in memory (`ClarifyRetention`, keyed by Capture id) for the life of a Ceremony performance, so it survives Skip and Back but not process death — accepted, since an in-progress draft was never a persisted thing.
- The codebase uses "Todo" (DB table, Drift class) and "Task" (docs, foreign-key columns like `task_id`) for what the model calls an **Outcome**. No Dart identifier says `nextActionText` any more — the one-field surfaces take `actionText` and are named for the Action they write (`TodoDao.setCurrentActionText` / `setCurrentActionTextIfActionless`), and the column itself is gone (ADR-0024, issue #525): dropped from `tables.dart`'s declarations, with only a `from < 28` migration step in `gtd_database.dart` (and the historical Alembic migrations) still naming the retired `next_action_text` string to drop it. The proposal at `docs/proposals/blockers-and-contexts.md` uses "Task" and "NextAction" throughout; that document predates these renames.
...
- **Area exclusivity is enforced at the cutover, not yet in storage.** The model says an Outcome belongs to at most one Area (ADR-0025). Storage still permits many, since Areas are `Tag(type='area')` rows linked through `todo_tags`, and the enforcement is not a one-line constraint: the join carries `todo_id` / `tag_id` and no `type`, so "at most one Area" is not expressible as a unique index on it — it needs either a dedicated Outcome→Area link (or a column on the Outcome) or a check that reaches into `tags`. What *is* settled is the migration: the **Initial Upload** applies ADR-0025's auto-resolve as it authors the store onto the op-log spine. An Outcome holding several Areas keeps the one whose (casefolded name, id) sorts first — a deterministic **provisional** pick — and each surplus membership becomes membership of the **Label** named after that Area, merging onto an existing Label of that name where one exists, so no categorisation is lost. Every conversion is enumerated in the upload's report, which is the worklist for the user's own resolution in the following Weekly Review. The ambiguity has therefore narrowed from "needs a migration" to two remaining pieces: **storage-level enforcement** (still unbuilt, still needing the schema decision above) and that **post-cutover user pass**.
- The current code calls the **Next** List "Next Actions" throughout (provider names, screen names, UI copy). The List is conceptually a List of Outcomes — the action focus comes from each Outcome's current Action — so the plural-Actions name misleads. Renaming target: **Next** in code, providers, and copy; the role-label "Next Action" stays for the singular current Action of a row in that List. The same List also carries **two user-facing names**: the drawer renders it "Next Actions", while the Daily Planning plan-building step renders it "Up Next". That divergence is deliberate — the step's pool is energy-filtered and net of what is already picked or skipped, so one shared name across the two surfaces would show two different counts under one label — but neither name is the model's, and "one user-facing name per domain List" stays an open question until the rename above settles it.
...
**Initial Upload** *(what signing up for sync does with the data you already have)*:
The whole local store, authored onto the op log the moment a Device becomes enrolled. It is **product flow, not a migration ceremony**: there is no button, no verification gate and nothing for the user to review — signing up for sync starts syncing, and the data accrued offline comes along. It is idempotent by construction (it compares each entity against what the log already holds rather than remembering the run), so an interrupted one resumes on the next sync, and a Device joining an account that already has data finds nothing to do. Verification is available afterwards, by choice. See ADR-0034; the ADR-0025 Area-exclusivity resolution it applies produces the worklist for the next Weekly Review.
_Avoid_: Reseed (the name of a cutover-scoped verification surface that no longer exists — #595 deleted it), Migration (a ceremony this deliberately is not), Backfill (implies filling a gap rather than carrying a store)
...
- **Nor does the *session*.** A server that cannot be reached — or that answers but is not identifiably the Jeeves backend — never signs a Device out. Only a `401` from the refresh endpoint corroborated as the backend's own may cost a Device its stored credentials; every other answer is inconclusive, and an inconclusive answer leaves the Device signed in and authoring. The reason this belongs here rather than only in the auth layer is that the account id is what every Workspace id, the escrow slot and the Grants derive from, so a wrongly-cleared session does not merely prompt for a password — it unbinds the capture seam and drops the session's ops. See ADR-0041.
...
- **A Member belongs to at most one User.** A **Device** belongs to exactly one; a **Service** to none (registered, not owned). A **Grant** joins one Member to one Workspace with one role; a **KeyWrap** fulfils a Grant for one epoch. The User's **Root** signs the whole control plane; **Recovery** escrows Root under the passphrase and is not a Member.

#### Flagged ambiguities

- **The op log is the whole client sync stack.** The Workspace security cluster is what runs in the app: a Device enrols on the passphrase alone, pins Root, founds or joins each of its User's two implicit Workspaces with a Root-signed control op, holds an explicit Root-signed **owner** Grant, verifies every author's chain and every authority on the way in, and seals content under the Workspace's current epoch key once an owner has turned encryption on. Enrolment is also what *starts* syncing — the **Initial Upload** authors the store the user already has, and from then on every domain write is an op (issues #591, #595; ADR-0034, ADR-0035). Implemented: **Workspace** as a genesis-signed partition, **Member**, **Root**, **Recovery**, **Author**, **Integrity Alarm**, **Grant** with its four roles, and **KeyWrap** with **Rotate** and the epoch floor.

- **KeyWrap, epochs and the `aead_v1` suite are built end to end (#554), and no Workspace is encrypted until an owner says so.** A Device holds its Workspaces' epoch keys, opens the wraps addressed to it, seals content under the current epoch, and runs both ceremonies — turning encryption on, and revoke-plus-rotate. Applying a verified **Rotate** is what raises the epoch floor, so the floor finally has a raiser and a suppressed rotation is detectable rather than a silent downgrade. The *orphaned grant* predicate fires for real, and is answerable about the Device's **own** Grants alone: a KeyWrap is sealed to one Member, so no Device can observe whether a peer was wrapped to.

Two things are deliberately not automatic. **Encryption turning on is a per-Workspace ceremony an owner runs with the passphrase, never a deploy** — the escrow wrap for a new epoch is minted under `master_wrap_key`, which lives behind the passphrase, which is also why *scheduled* rotation surfaces as a prompt when an epoch has stood too long rather than running unattended. And a newly founded Workspace is keyed at genesis only when the founding ceremony is asked to; the default leaves it `plaintext_v1` so nothing about an existing deployment changes, and the byte-inspectable log the plaintext cutover (#553) verified against stays readable for ever.
...
The unit of partition, encryption, and replication — one Inbox, one set of Areas, one Next List, one Plan. Not a subset of a User's data but a whole system, analogous to Allen's advice to keep a functional workstation at the office and another at home, each with its own in-tray and filing (GTD ch. 4). A User has two implicit Workspaces today — the default GTD one and a User-global `user_preferences` one — each addressed by a `uuid5` derivation of the user id so N devices agree on the id with no round-trip, and each brought into existence by a `workspace_genesis` control op that embeds its founding Device's registration (ADR-0031). Each Workspace has its own key and its own ordered stream of changes, and access is granted per Workspace — which is what lets a User expose one Workspace to a Service while keeping another private.

**A Workspace is one mind's system.** Its Inbox, Next List and Plan hold personal *stance* — Intent, clarification stamps, what the User chose to do today — which has no shared meaning across two people. The standing position is therefore **delegation over co-ownership**: work is handed to another User by copying the Outcome into *their* Workspace while the sender keeps a PersonBlocker and tracks it on Waiting For, exactly as GTD models handing something off. The copy carries the Outcome's content — title, notes, Actions — never the sender's organisational links: Areas, Labels, and Person references are Tag-backed and therefore Workspace-owned, so they stay in the sender's Workspace and the recipient clarifies the copy into their own structure. The sender's PersonBlocker for the handoff references the sender's own Person entry for the recipient, exactly as any other Waiting For. Co-ownership would require splitting every entity into a shared *substrate* (Captures, Outcomes, Actions, Tags, Areas — the *what*) and a per-User *stance* overlay living in each participant's own Workspace and referencing the substrate across the boundary. That is buildable on the existing cross-Workspace reference rule and needs no new transport, but it is a GTD Core model change, not a Sync one, and it is not the current direction.

**Isolation is a property of storage and membership, not of the User's view.** Every Device is granted every Workspace its User participates in and therefore holds every key its User holds, so a client may freely render across them — a day's Plan is a union of that day's per-Workspace FocusSessions, presented together. A grantee holding one key cannot: it resolves references into Workspaces it lacks as *elsewhere*, never as missing. Enforcing isolation in the UI would be enforcing it against the one party it is not meant to constrain.

**A Capture always lands in the current Workspace, unprompted** — capture must never ask a question. Relocating it is a clarification verdict alongside carve / merge / discard, and it is mechanically a copy rather than a move: re-encrypt under the destination key, sign, write, then tombstone the source. Relocating an already-clarified Outcome, with its Action history and TimeLogs, is the same operation at far greater cost and is correspondingly rare.
...
An entity that can hold Workspace keys, author Ops, and authenticate to the sync server — identified by its registered public keys (sequences of signing and key-exchange keys with ids and validity windows, not one immutable keypair). Two kinds. A **Device** is owned by exactly one User (enrols by unwrapping the User's Recovery escrow with the passphrase, then self-registers under the User's Root) and is granted every Workspace its User participates in automatically. A **Service** is *registered, not owned* — it belongs to no User; its long-lived identity key is pinned outside the sync server (app-binary trust store, or user-confirmed fingerprint for self-hosted) and signs short-lived per-User key-exchange subkeys. Users relate to a Service only through Grants, one named Workspace at a time.
...
A Workspace's symmetric key for one **epoch**, wrapped to one Member's key-exchange key — one KeyWrap per (Member, epoch). Key rotation (on revocation, and on schedule) mints the next epoch and re-wraps for the survivors; revoke and rotate are one atomic act. Epochs are monotone, bound into every Op's authenticated header; clients hold a persisted epoch floor and refuse to write below it, which is what makes a suppressed rotation detectable rather than a silent downgrade. Historical epoch keys are kept for ever: soft-delete retention means content authored at any past epoch may still have to be read.

A KeyWrap has a second **flavour**, the **escrow wrap** — the same epoch key wrapped under the User's `master_wrap_key`, one per (Workspace, epoch) rather than per Member. It is what lets a Device recover all history from the passphrase alone, so the enrolment invariant "no second device online, ever" survives encryption. The server can open neither flavour (ADR-0037).

Minting the wraps and authoring the **Rotate** that announces them is one **ceremony**, and the order inside it is load-bearing: the Rotate commits to a digest over the whole wrap set *before* any wrap is uploaded, so the server can neither add a wrap for a Member the owner never wrapped to nor omit one and lock an honest Member out. Turning encryption on for an existing Workspace *is* a rotation — never a separate switch — and it mints the key for epoch N+1, leaving the earlier unkeyed epochs' plaintext history readable for ever.
_Avoid_: Wrapped key (acceptable colloquially), Key share, Grant (the authorization fact KeyWrap fulfils)

**Rotate** *(the Workspace moved to a new content key)*:
The control op that announces a new **epoch**: `from_epoch`, `to_epoch = from_epoch + 1`, and the digest committing to that epoch's whole KeyWrap set. Single-step by construction — a jump would leave an epoch no KeyWrap set is ever minted for. Alone among control types it carries **no certificate**: every other type's authority is somebody other than the envelope's author, whereas a Rotate's authority is the author's own live **owner** Grant, so the envelope signature is the only signature there is. Applying a verified Rotate is what raises a client's epoch floor.
_Avoid_: Key rotation (the ceremony, of which the Rotate op is one part), Member key rotate (a different, unserved concept: rotating a *Member's* own signing or KEX key)

**Root** *(a User's signing authority)*:
A random Ed25519 keypair generated at account creation and escrowed under the User's passphrase — never derived from it, so a passphrase change is a re-wrap, not a new identity. Root signs the control plane: member registrations, Grants, revocations, rotations. Devices pin the Root public key on first successful escrow unwrap (trust-on-first-use against the *passphrase*, not against the server) and refuse any Member not chained to it. This is what makes the sync server unable to inject members or substitute keys.
_Avoid_: Master key (Root signs; it does not encrypt), Identity key (a Member-level term), Passphrase (the thing that unlocks Root, not Root itself)

**Recovery** *(the passphrase escrow — not a participant)*:
A constant-size blob — the Root private key plus a master wrap key, under Argon2id of the User's passphrase — stored server-side, root-signed, monotonically versioned, with client-enforced KDF floors; per-epoch Workspace keys are stored wrapped under the master wrap key. Recovery is how a new Device bootstraps with only the passphrase and no second device online. It is a property of the User, not a Member: no keypair identity, no cursor, no authorship. Passphrase-derived material never leaves the device. The passphrase is user-chosen with a generated diceware default; its entropy is the E2EE ceiling against a stolen-ciphertext adversary, so the override path warns rather than silently accepts.
...
**Author** *(a role, not an entity)*:
The Member whose signature is on an Op. Authorship is bound into the authenticated cleartext header (member id, key id, per-author sequence number, hash of that author's previous op), so co-members cannot reattribute ops to each other, and server truncation, reordering or rollback of any author's stream is *detected* by every receiving Device rather than merely being detectable in principle — see **Integrity Alarm**. "Author" names the relationship between a Member and one Op; it never names a kind of Member.
_Avoid_: Writer, Creator, Owner (a Grant role), Sender

**Integrity Alarm** *(a standing accusation, or a standing local condition that will never heal)*:
What a Device records when the log it was served cannot be reconciled with the per-author chains bound into it, or with the keys it holds: a withheld op, a reordered or forked stream, a rewritten position, a stale prefix, a rollback of the Device's own writes, a divergent duplicate, a signature that does not verify, an `aead_v1` body that does not authenticate under an epoch key the Device *holds*, or a plaintext content op at an epoch that has a key. The last two are the encryption half, and the refusal that is pointedly **not** among them is a missing epoch key: a KeyWrap that has not arrived is a delivery gap that heals, not an accusation — even when the gap is permanent because the Device was revoked. Most alarms name a culprit, and one kind deliberately does not: a prepared **KeyWrap** set for an epoch this Device can provably never publish is a standing *local* condition, raised because it will never heal itself and not because anyone misbehaved — the server was right to refuse it. One row per `(Workspace, kind, author)`, counted while unresolved and resolved when the accusation stops standing — a reorder that healed is resolved; a genuine drop never is, and neither is a condition with no heal.

**An accusation standing is not the same as the User being told something is wrong, and most standing accusations are records of the app working.** Fourteen of the eighteen kinds mean the fail-closed rule did its job and nothing of the User's is at risk — a forged envelope refused, a reorder that converged, a duplicate ignored. Only four mean something of the User's is stuck or lost, and only those four turn the indicator red. The distinction is the **Sync Condition Class**, below; it is a property of the kind, decided at design time and stored nowhere, so it is never something a gesture can set.

Distinct from **Quarantine**, the per-op record of *one refused envelope*: one alarm can cover many refused envelopes, and a rollback alarm has no refused envelope at all because it is detected when the server rejects the Device's own append. Both are the successor to the retired **Dead Letter**: a refused envelope is evidence kept in the log, not a row in a side table nothing drains.
_Avoid_: Sync error (a transport failure of one attempt is not an accusation), Conflict (merge conflicts are resolved by LWW and are not integrity events), Dead Letter (retired vocabulary), Dismiss (withdrawn — silence is not an outcome; a conflict is either settled or it stands)

**Quarantine** *(the per-op record of one refused envelope)*:
Where a Device keeps an envelope that failed a fail-closed rule — never applied, always surfaced, kept rather than deleted so the user can inspect it. One row per refused envelope, carrying the reason the rule gives and the author position it claimed, which is what lets a chain refusal be re-admitted (**released**) once its missing predecessor arrives: a hostile reorder that healed is still something worth showing. Distinct from **Integrity Alarm**, the standing accusation: one alarm can cover many quarantined envelopes, and some alarms have none at all. A refused envelope is nonetheless kept in the received log as chain evidence — quarantine decides what is *applied*, not what is *remembered*.

**A refusal is never an error to the User.** The bytes were refused, which is the outcome the rule exists to produce. Five reasons are not even worth telling them about — a KeyWrap or an epoch that has not arrived yet is a delivery gap that heals on its own, and interrupting for it is what a Device used to do.
_Avoid_: Dead Letter (retired vocabulary, and for an unrelated mechanism), Rejected op (a rejection is the server's answer to an append, not the receiving Device's record), Discarded (nothing is discarded)

**Sync Condition Class** *(what a standing condition is to the User)*:
The three-way classification every **Integrity Alarm** kind and every **Quarantine** reason carries: `actionable` (something of the User's is stuck or lost, and no mechanism in the app fixes it), `reported` (the app already did the right thing and nothing is at risk), `transient` (waiting on delivery, heals by itself). Fixed at design time as a property of the kind, computed from the stored code and persisted nowhere — never a state a gesture can set, which is what separates it from dismissal. Four alarm kinds are `actionable`, fourteen are `reported`, and five refusal reasons are `transient` (ADR-0044).
_Avoid_: Severity (this is what a condition *is*, not how loud it is), Acknowledged (nothing is acknowledged; the class does not move)

**Sync Health** *(what the indicator claims, and what the sync screen accounts for)*:
The three lines the surface holds to. **Reachable** — the indicator is tappable if and only if there is something to report: any unresolved **Integrity Alarm**, or any unreleased **Quarantine** row whose reason is not `transient`. A Device with nothing to report has no entry point at all, deliberately: a permanent way to check on a subsystem that is working is a standing invitation to worry about it. **An error** — the indicator is red if and only if an `actionable` alarm is standing. Nothing else drives it, and across the Device's two Workspaces the worst *undecided* news wins: one Workspace in the calm band beside another holding an `actionable` alarm renders red. **Worth knowing** — the state between them, where fourteen of the eighteen kinds live: visible, non-red, nothing to do. It exists because a reachable screen behind a healthy-looking indicator would make the entry point a secret rather than a surface, and it tells the User in one sentence that this is worth reading and needs nothing from them.
_Avoid_: Sync status (that is the indicator's five-way display state, not the account behind it), Degraded (retained in code as the error predicate; not user-facing vocabulary)
...
**A Tag has two identities, and only one of them is the primary key.** `id` is the op-log identity — client-random, so it is what the sync substrate names the entity by. `(name, type)` is the *user-facing* identity: two rows with the same pair are one Person, one Area, one Label or one Context as far as the user is concerned — **every concept Tag backs, with no exemptions**, because the fold groups on the pair and never reads which concept the `type` names. Because ids are random, two Devices each creating "Alice"/`person` while apart mint two entities for one pair, and both are real. **That state is transient and converging, not corruption** — the storage has no uniqueness constraint on the pair precisely so it can be represented, and the Tags collapse onto one (the lexicographically smallest `id`) as the Devices converge, carrying every Outcome link and tag hint with them. A user who sees two "Alice" entries for a moment on a freshly reconnected Device is watching convergence, not a bug; a user who still sees them once their Devices have synced **is** looking at a bug — the fold did not run, or its ops never reached them (ADR-0043).
...
**Last-Write-Wins (LWW)** *(the default merge rule)*:
The default rule for merging concurrent edits, applied at **field grain** by the reducer and ordered by **HLC** — not by `updated_at`, which is an ordinary replicated field a device could write out of order. Two devices editing different fields of the same entity therefore both keep their edit; only a genuine same-field conflict is arbitrated, and the winner is the higher HLC with the member id as the final deterministic tie-break. Acceptable for the current single-user-many-devices model; future collaboration features may require operation-based or CRDT-style resolution on the shapes they touch. For `user_preferences` it is the default entry of the **Conflict Strategy** registry, and a few keys deviate from it. ADR-0030 constrains every strategy, LWW included, to be a join semilattice: merge has to be commutative, associative and idempotent, or devices that receive the same ops in different orders do not converge.
_Avoid_: Last-write-wins (capitalisation drift), Latest wins, Newest wins, Row-level LWW (the grain is the field)

**Conflict Strategy** *(per-key `user_preferences` reconciliation rule)*:
The rule that decides which value a `user_preferences` row holds after a local and a server copy conflict. Defined per key in a code registry (`services/user_preferences_conflict.dart`) so the policy is an auditable contract read by the reducer rather than a rule buried in whatever merges the rows, and so future keys register a strategy rather than editing prose. Three strategies exist: **LWW** (default, non-destructive; scalar keys), **max-timestamp-value** (snooze "until" floors — between two live values the later one wins so a floor never regresses, while a clear/un-snooze is arbitrated against a live value by last-write-wins on `updated_at`), and **set-merge** (union of list values so concurrent additions survive; provisioned, no key uses it yet). The default must be non-destructive for any future key. On the op log the strategy-selecting key travels on every value write and an op that omits it is refused, not defaulted, so which strategy arbitrates a write never depends on what a device has already learned (ADR-0033). The full matrix lives in `docs/SYNC.md`; see also ADR-0011.
