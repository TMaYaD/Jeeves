...
- **Today's Nudge precedence is implemented as a Banner-only suppression** (PR #308): `FocusSessionPlanningBanner` self-suppresses when `periodicReviewBannerVisibleProvider` is true. This is the WR > DPR ordering applied at one specific surface. The conceptual Nudge queue applies the rule to every surface — the Daily Planning Notification should also yield while Weekly Review's Nudge is at the queue head, but today's OS-scheduled `matchDateTimeComponents: time` Notification fires regardless. Reconciliation is part of the Nudge consolidation work above.
- **Wizard machinery is currently duplicated per Ritual** (`FocusSessionPlanningScreen`, `ShutdownRitualScreen`, `PeriodicReviewScreen`): each owns its own `PageController` plumbing, segmented progress bar, and Skip/Next footer, with only cosmetic divergence across the three. The Wizard implementation form is named at line 370 ("A Ceremony may be implemented as a Wizard — current default for all three Rituals") but lives as three bespoke implementations rather than one Ritual-parametrised abstraction. The wizard-footer contract is normative — `docs/DESIGN.md:119–135` prescribes a single Skip + Next-step affordance with one fixed slot — and the per-item-cursor advance/retreat behaviour is spec'd as one contract across rituals in `docs/REQUIREMENTS.md:72`. The drift across the three current footers is therefore implementation noise, not feature variation. Consolidation target: a single Wizard widget consuming a list of Step components, with the DESIGN.md footer contract enforced once. Same shape as the Banner/Notification consolidation above — duplicated-per-Ritual surface machinery folded into one Ritual-parametrised abstraction.
