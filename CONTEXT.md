...
**The Action rows are the only grain.** The `actions` table is where an Action's existence, text, role and effort metadata live, and it is the only thing the app reads or writes to answer "what is this Outcome's next move?". The legacy one-field cursor, `todos.next_action_text`, is **retired by abandonment** (ADR-0022): it stays declared and keeps replicating so a pre-retirement client still syncs, but nothing in the app touches it, and no invariant ties it to the Action rows any more. Where a cursor written by such a client disagrees with the Action rows, the Action rows win — nothing reconciles the disagreement, and an Outcome synced in from a client that predates the Actions table, carrying only a cursor, renders Actionless. The startup sweep is cursor-free: it never reads the cursor and never adopts one into an Action. Its one pass, `convergeMultiCurrentActions`, only repairs an accidental multi-`current` set by retiring the losers to a single winner under the same deterministic rule the writers use — it reads `actions` and nothing else.
...
- The codebase uses "Todo" (DB table, Drift class) and "Task" (docs, foreign-key columns like `task_id`) for what the model calls an **Outcome**. No Dart *parameter* says `nextActionText` any more — the one-field surfaces take `actionText`, and the only surviving `nextActionText` identifiers are the two that name the retired column itself: its declaration in `tables.dart` and its migration step in `gtd_database.dart` (ADR-0022 keeps the column declared and replicated but neither read nor written). The proposal at `docs/proposals/blockers-and-contexts.md` uses "Task" and "NextAction" throughout; that document predates these renames.
