...
- **Area exclusivity is conceptual-only so far.** The model says an Outcome belongs to at most one Area (ADR-0025); storage still permits many, since Areas are `Tag(type='area')` rows linked through `todo_tags`. Nor is the enforcement a one-line constraint: the join carries `todo_id` / `tag_id` and no `type`, so "at most one Area" is not expressible as a unique index on it — it needs either a dedicated Outcome→Area link (or a column on the Outcome) or a check that reaches into `tags`. Reconciliation needs that enforcement plus a migration over Outcomes that currently hold several — resolved by user choice, with the surplus memberships converted to **Labels** rather than dropped, so no categorisation is lost.
