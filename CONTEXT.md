...
**Integrity Alarm** *(a standing accusation against the server or an author)*:
What a Device records when the log it was served cannot be reconciled with the per-author chains bound into it, or with the keys it holds: a withheld op, a reordered or forked stream, a rewritten position, a stale prefix, a rollback of the Device's own writes, a divergent duplicate, a signature that does not verify, an `aead_v1` body that does not authenticate under an epoch key the Device *holds*, or a plaintext content op at an epoch that has a key. The last two are the encryption half, and the refusal that is pointedly **not** among them is a missing epoch key: a KeyWrap that has not arrived is a delivery gap that heals, not an accusation — even when the gap is permanent because the Device was revoked. One row per `(Workspace, kind, author)`, counted while unresolved and resolved when the accusation stops standing — a reorder that healed is resolved; a genuine drop never is. Distinct from **Quarantine**, the per-op record of *one refused envelope*: one alarm can cover many refused envelopes, and a rollback alarm has no refused envelope at all because it is detected when the server rejects the Device's own append. Both are the successor to the retired **Dead Letter**: a refused envelope is evidence kept in the log, not a row in a side table nothing drains.
