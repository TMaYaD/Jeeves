...
Minting the wraps and authoring the **Rotate** that announces them is one **ceremony**, and the order inside it is load-bearing: the Rotate commits to a digest over the whole wrap set *before* any wrap is uploaded, so the server can neither add a wrap for a Member the owner never wrapped to nor omit one and lock an honest Member out. Those two steps — flush the Rotate, then upload the wraps — are not atomic, so the prepared wrap set (member wraps, escrow wrap, digest and the epoch key itself) is kept **durable on the Device's keychain tier before the flush**, keyed by (Workspace, epoch): a crash in the gap resumes on the next ceremony, pull or launch as a byte-identical re-upload the server acknowledges, rather than stranding an epoch nobody holds the key for, and the record is dropped once the key is remembered. Re-minting cannot heal such a gap — fresh entropy yields a digest the committed Rotate can never match — which is exactly why the set is preserved rather than rebuilt. Turning encryption on for an existing Workspace *is* a rotation — never a separate switch — and it mints the key for epoch N+1, leaving the earlier unkeyed epochs' plaintext history readable for ever.
