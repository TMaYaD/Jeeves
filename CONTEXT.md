...
- **`clarified` (boolean)** on the Todo row is a leftover of the Capture/Outcome conflation. Nothing reads it any more — the Inbox is `captures.clarified_at IS NULL` — but it is still *written*, because `TodoDao.applyRouting` sets it when an Outcome is created, and old clients during the staged rollout keep writing `clarified = false` rows that the release-train migration sweeps. The column is retained (unread) through one release train and dropped in a follow-up migration. **`last_clarified_at`** is a distinct concept — staleness of the *current* clarification — and survives the split.
