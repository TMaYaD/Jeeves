...
- The app now runs on the split model end to end: the Inbox reads `captures WHERE clarified_at IS NULL`, quick-add writes a Capture, and every clarify surface goes through `ClarificationService.clarifyCaptureToOutcome` / `discardCapture` — creating an Outcome, writing the `capture_outcomes` provenance link, and stamping `clarified_at` (ADR-0006, issue #184). Clarification currently runs in **1-1 mode** only; n-m split/merge is the conceptual model the storage already supports but the UI does not yet expose.
