...
- **Content-state dismiss is per-firing only by approximation.** Dismiss is scoped to the current firing (persisted in the Nudge module's `NudgeState`), so for Cadence-only Rituals (Daily Planning, Evening Shutdown) re-firing surfaces the Nudge again as the model intends, and Nudge precedence is applied uniformly across every surface — Banner *and* Notification — by the Nudge queue's priority ordering. Weekly Review's content-state Trigger should refire on each false→true predicate edge; today that per-firing edge is approximated by a stable per-day stamp rather than tracked reactively. Full reactive edge tracking for content-state Triggers is the remaining future pass.
