...
- A reference to the engaged Action (conceptually required; the stored column is nullable only to admit pre-Action-era logs, whose current-Action is unreconstructable — a null there means "no Action attribution available", never a TimeLog that conceptually lacks an Action)
