...
- **`user_preferences` is User-global, not per-Workspace.** It lives in a single implicit Workspace every Device is granted and no Service ever is.
...
The unit of partition, encryption, and replication — one Inbox, one set of Areas, one Next List, one Plan. Not a subset of a User's data but a whole system, analogous to Allen's advice to keep a functional workstation at the office and another at home, each with its own in-tray and filing (GTD ch. 4). A User has one Workspace today. Each Workspace has its own key and its own ordered stream of changes, and access is granted per Workspace — which is what lets a User expose one Workspace to a Service while keeping another private.
