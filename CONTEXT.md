...
- The **Action** entity now exists as first-class rows (`actions` table, ADR-0001) and is *written* on every next-action write path (issue #472 story 2): `ActionDao` maintains the Outcome's `current` Action — create, in-place text edit, explicit supersession without linkage metadata (ADR-0018), and clear — and the legacy cursor-field writes dual-write it. *Reads* of the current Action — whether an Outcome has one, and its text — now come from `actions` too (issue #473 story 3), so `next_action_text` is write-only compatibility until story 9 removes it. **Completion is now a real transition** (issue #474 story 4): `ActionDao.completeCurrentAction` flips the current Action to `role='done'` with `done_at`, clears the cursor in the same transaction, and deliberately does not stamp `last_clarified_at` — so the Outcome enters the no-current-Action state owing a re-clarification (ADR-0004). Achieving the *Outcome* cascades into the same transition on both write paths that set `done_at`. Freshness reads those terminations directly: Stale now fires on the later of a session close (`last_next_action_completion_at`, kept as the "worked on in a session" signal — history no Action row can reconstruct) and the latest `done` Action's termination timestamp. **Energy level and time estimate are now Action-grain** (issue #477 story 7): they describe the *action of doing*, so a new current Action never inherits the effort metadata of the one it replaced. Reads resolve to the current Action's value with the Outcome column as fallback (Actionless and legacy rows keep displaying their stored value), `TodoDao.updateFields` dual-writes the current Action while keeping the Outcome column mirrored, and supersession freezes the retired row's values while mirroring the replacement's onto the column so the startup sweep stays a no-op. `time_spent_minutes` is the one metadata cursor-field still to move (issue #476 story 6). The Outcome's `energy_level` / `time_estimate` columns remain as write-mirror compatibility until story 9 retires them; a startup sweep repairs any residual cursor→Action drift. **The plan is now real** (issue #475 story 5): an Outcome's planned Actions are `role='planned'` rows ordered by a dense `position`, shown only in the Outcome's detail view and never engageable (every current-Action read filters `role='current'`). `ActionDao` owns the queue — add / reorder / promote / supersede-and-promote / demote / remove, each a stamping clarifying act; promotion writes the cursor and demotion clears it, so the sweep stays stable. **History is now a surface** (issue #478 story 8): the Outcome detail view renders its terminated Actions — `done` and `superseded` alike — newest-first by `COALESCE(done_at, updated_at, created_at)`, each with the minutes logged against that Action, in a collapsed-by-default section under the Plan that is hidden when the Outcome has terminated nothing. The section is read-only by construction, and an explicit **Abandon** affordance on the current-Action row retires it into that history with no replacement (`ActionDao.clearCurrentAction`, which also clears the cursor so the sweep cannot resurrect it). The remaining reconciliation — time-spent wiring (story 6) and the naming rename (story 9) — lands next. Until then the cursor-fields and the Action rows coexist, kept in agreement by the dual-write and the sweep.
