...
- **Startup sweep convergence.** The startup `reconcileActionsWithCursor` sweep has two passes, and both are safe to run on every device independently. `convergeMultiCurrentActions` retires the losers of a multi-`current` set by the deterministic winner rule, so devices that raced collapse onto the same row. `adoptCursorsWithoutActions` is **mint-only**: it inserts one `current` Action for a live Outcome (`done_at IS NULL` — completion is the one transition that terminates Actions; trashing is not, so a trashed Outcome is still adopted and its next-action text survives a restore) that carries a non-blank `next_action_text` **and no `actions` rows at all**, at the deterministic backfill id `backfillActionIdFor(todo_id)` — so two devices adopting independently upsert onto the **same** id (create-dedupe / ADR-0015) rather than minting divergent random-id `current` rows that would break the 0..1-current invariant on sync. Because that id is computed client-side, the pass first reads back which candidate ids are already occupied and skips those Outcomes: a legacy row carrying a NULL `outcome_id` satisfies the pass's `NOT EXISTS` guard while still holding the primary key, and a bare insert on top of it would abort the startup transaction. The sweep never overwrites and never retires on the cursor's word. A cursor edit replicated from a client that predates the Actions table is therefore ignored on any Outcome that already has Action rows — an accepted divergence, taken because the alternative (letting a stale cursor overwrite or retire a `current` Action) destroys Action-grain history and its `time_logs` attribution.
