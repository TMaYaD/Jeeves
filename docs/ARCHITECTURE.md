...
- **Outcome routing** (review surfaces, and today's Inbox flows) — `clarifyToOutcome`, `completeOutcome`, `stampClarified`, `updateFields`, plus the `exists` / `getPersonTagIds` reads the flows need to guard their writes. `DaoClarificationService` delegates these 1:1 to the `TodoDao` (and, for the capture-existence read, `CaptureDao`) methods below.
