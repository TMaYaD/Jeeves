...
The scheme rests on **blank cursor ⟺ no `current` row** as a guarantee this app makes about *its own* writes, not as a property of the stored data. Every role transition owns its own cursor write rather than relying on a choke point above it: promote sets the cursor to the promoted text (plus the metadata mirror, D4 below), demote and completion clear it, and `applySupersedeCurrentAction` writes it both ways — the replacement's text when one is minted, NULL when the Action is abandoned outright. Supersession's clear arm matters most, because it is what keeps a *cursor-only* client — one that predates the Actions table — from later reading the Outcome as still having that next action. The startup sweep can no longer undo an abandon: its adoption pass mints only into an Outcome with no `actions` rows at all, and the retired row is one. The write is idempotent for the `TodoDao` choke points, which write the identical value at the same timestamp before calling in.

A peer that predates the Actions table writes the cursor alone, so the store can hold the correspondence broken in either direction — a cursor such a client blanks leaves this device's `current` Action standing, and a cursor it fills leaves an Outcome that already has Action rows unadopted. Nothing repairs either case, deliberately: the sweep no longer retires on a blank cursor and never adopts over existing rows (see the residual exposure below). Read the Action row wherever the two disagree.
