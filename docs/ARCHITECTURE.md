...
- **`adoptCursorsWithoutActions` — cursor-dependent, mint-only.** Mints one `current` Action, at the deterministic backfill id `backfillActionIdFor(todo_id)` (ADR-0019, so independent devices converge on one row rather than minting divergent random-id rows), for a **live** Outcome that carries a non-blank `next_action_text` **and has no `actions` rows at all**. The `NOT EXISTS` guard spans every role, and it is the whole safety property: the pass can never resurrect an abandoned Action, revive a completed one, or overwrite an edited one. Live means `done_at IS NULL AND intent != 'trash'` — the states in which a write path here can hold a `current` Action. Completion terminates the current Action (story 4), so minting onto a completed Outcome would manufacture a state the app never writes; trashing is the user's explicit discard. Someday/Maybe and unclarified Outcomes are kept, because `applyRouting`'s `maybe` arm leaves both the cursor and existing Action rows alone and adoption is a one-shot rescue that dies with the columns — skipping a deferred Outcome would lose its next-action text for good. Waiting For needs no clause: it is `intent = 'next'` plus a `Tag(type='person')`, not a separate intent. The pass exists only to adopt an Outcome created by a client that predates the Actions table, and dies with the columns.
