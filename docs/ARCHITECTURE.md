...
1. Loads the Capture from the local DB via `CaptureDao.getCapture`, along with its non-person tag hints (`CaptureDao.tagHintsForCapture`) so they can seed the new Outcome's tags. Both are one-shot reads, not watches: the screen renders neither live, so there is nothing for a stream to keep in step.
