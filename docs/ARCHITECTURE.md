...
**Dual-write:** every legacy path that sets `next_action_text` now also drives `ActionDao` in the same transaction with the same timestamp, and keeps its cursor write byte-identical — `TodoDao.setNextActionText` (blank routes to `clearCurrentAction`), `TodoDao.applyRouting`'s `nextAction` / `waitingFor` arms (blank → clear, absent → no-op; the other arms write no Action), and `TodoDao.deleteOutcome` (carve-undo cascades the Outcome's Action rows explicitly — the local view enforces no FK cascade). The cursor and the Outcome's `current` Action therefore agree after every write. One deliberate gap remains: `TodoDao.updateFields` (`todo_dao.dart`) edits the Action cursor-fields `energy_level` / `time_estimate` on the Outcome but does **not** yet dual-write them to the Action row — live metadata wiring is **story 7**. Until then an Action's metadata drifts after an `updateFields` edit and is repaired by the next startup sweep (mode 1). That gap is invisible today because **metadata reads have not moved**: energy and time-estimate are read from the Outcome columns `todos.energy_level` / `todos.time_estimate`, which `updateFields` keeps accurate, and nothing reads `actions.energy_level` / `actions.time_estimate`. Story 3 cut over only the **text and existence** reads, whose every write path dual-writes atomically; **metadata reads must not move to the Action row until story 7 wires the metadata dual-write.**
...
**Completion (story 4, issue #474):** `ActionDao.completeCurrentAction(outcomeId, {now})` is the transition that records *this Action* as finished: it converges any multi-current set, flips the winner to `role='done'` with `done_at`, and leaves the Outcome active and Actionless (ADR-0004 — nothing is auto-promoted). It is the single exception to the stamping rule: completion is an **engagement** signal, not a clarifying act (CONTEXT.md § Clarification), so `last_clarified_at` stays put and the Outcome immediately owes a re-clarification. Two consequences follow. First, the same transaction **clears `todos.next_action_text`** — mandatory, not hygiene: the startup sweep's mode 3 reads "non-blank cursor + no current row" as drift and mints a fresh `current` Action, so a populated cursor would resurrect the Action the user just finished at the next launch; clearing it preserves the blank-cursor ⟺ no-current-row invariant. Second, because that cursor write touches `todos` *without* stamping, the `todos` view-notify cannot ride on the `stamped` flag — `completeCurrentAction` notifies both views whenever it changed anything. It is a no-op on an Actionless Outcome and idempotent on replay (a second call finds no current row, so a completion can never produce two terminal rows or push `done_at` forward).
...
The residual exposure is a cursor-only change arriving from a client older than story 2: it re-emits the `todos` watchers while the reads come from an unchanged Action row, so a stale subtext or a wrongly-Actionless Outcome can render mid-session. The startup sweep repairs it at the next launch, pre-watchers. This is why story 2 must ship in a release train *before* story 3.

**Reconciliation sweep:** `reconcileActionsWithCursor` (`migration_service.dart`) runs at startup from `powerSyncInstanceProvider`, right after `migrateLocalInboxToCaptures` and before any watcher exists (so it needs no view-notify). It repairs, strictly in the direction cursor → actions, the two sources of drift: the #471 one-time backfill snapshot, and the ongoing window where an old app version replays `PATCH todos.next_action_text` (cursor-only) during the dual-write rollout. Three modes: **mode 1** brings a stale `current` row's text/metadata in line with the cursor; **mode 2** retires (not deletes — ADR-0018 history) a phantom `current` row left after the cursor was cleared; **mode 3** mints the missing row via the deterministic backfill id `backfillActionIdFor(todo_id)` (ADR-0019, so independent devices converge on one row), or, if that id is a retired row, resurrects it — guarded on `role = 'superseded'` so a future `done` row is never revived, which also self-heals the transient both-superseded convergence race (two devices mutually retire under sync lag → the next sweep flips the deterministic row back). The sweep is idempotent and clarification-neutral: it **never** stamps `last_clarified_at` (ADR-0012 spirit — never auto-stamp on drift).
...
- **Outcome routing** (review surfaces, and today's Inbox flows) — `clarifyToOutcome`, `promoteCaptureToOutcome`, `completeOutcome`, `completeCurrentAction`, `stampClarified`, `updateFields`, plus the `exists` / `getPersonTagIds` reads the flows need to guard their writes. `DaoClarificationService` delegates these 1:1 to the `InboxDao` / `TodoDao` / `ActionDao` methods below. `completeCurrentAction` is the odd one out and deliberately so: it is an engagement write that does *not* stamp `last_clarified_at` (ADR-0001 story 4), exposed here because the Focus "Done" flow completes the Action and then takes its re-clarification verdict through the sibling methods on this same interface.
