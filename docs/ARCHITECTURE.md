...
- **`backend/app/sync/`** — `ops` (append-only per Workspace, `seq BIGSERIAL` as a transport cursor only) and `members` (public keys, no authority), plus three endpoints: `POST /members`, `POST /w/{w}/ops` (batch, idempotent by author-namespaced op id, `author_seq` contiguity enforced, envelopes below the framing minimum refused), `GET /w/{w}/ops?since=&limit=`. Both uniqueness rules — `(workspace, author, op_id)` and `(workspace, author, author_seq)` — are database constraints, because the handler reads `MAX(author_seq)` before it inserts and two concurrent batches would otherwise fork an author's chain; a raced insert comes back as the idempotent duplicate or a 409, never a 500. Alembic `0031` adds both tables; neither joins the PowerSync publication. The server parses the 158-byte header for its index columns and its content-blind authz, and never a body.
...
Stubs this slice leaves for its siblings: the Root chain and member-scoped JWTs (#548), Workspace genesis and Grants (#549) — a User reaches exactly one implicit Workspace, `uuid5` of the user id — the remaining collections and ADR-0011's Conflict Strategy registry (#550), chain verification and integrity alarms (#551), the realtime signal (#552), and compaction/prune (#555; `ops.compacted_by` exists and is unused).
