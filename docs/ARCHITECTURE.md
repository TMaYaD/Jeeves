...
### Async surfaces (loading / error / empty / missing)

Two builders own every non-data state the UI can show, so the same situation looks the same wherever it arises. Both live in `app/lib/widgets/` and share `state_surfaces.dart` (`ErrorSurface`, `EmptySurface`).

- **`AsyncList<T>`** (`async_list.dart`) — list-bound screens (Inbox, Next Actions, Waiting For, Someday-Maybe, Search). Loading → spinner; error → fixed copy; empty list → icon + per-callsite title + optional subtitle/CTA.
- **`AsyncSubject<T>`** (`async_subject.dart`) — surfaces bound to a *single* row watched by id: both clarify surfaces, the task-detail screen, the active-focus sprint.

`AsyncSubject` exists to enforce one invariant: **`AsyncData(null)` from a subject watch means the row is gone, not that it is still loading.** `CaptureDao.watchCapture` and `TodoDao.watchTodo` use drift's `watchSingleOrNull()`, which emits `null` for zero rows and keeps the stream open; the app has no soft-delete (Trash is an `intent` value, so a trashed row still emits non-null), so the only way a subject becomes null is a hard delete — typically sync applying a remote delete under an open screen. Collapsing `AsyncValue<T?>` into a `value == null -> spinner` check is the anti-pattern the widget exists to prevent: it leaves a user whose item was deleted on another device staring at a spinner that never resolves.

Two further rules ride along. Error copy never leaks the exception — the detail goes to `debugPrint`, the user sees `ErrorSurface`'s fixed text. And every missing state offers a way out (`missingCta`, or `missingBuilder` for a host that would rather redirect, as `ActiveFocusScreen` does), because a surface bound to a row that no longer exists is otherwise a dead end.

Branch order is error → loading → missing → data, and **both** builders test the error ahead of `AsyncValue.when`. Riverpod 3 auto-retries a failed provider, so a stream error does not arrive as `AsyncError` — it arrives as `AsyncLoading` *carrying* an error, and `when` returns its loading branch for any loading state that is not a refresh/reload. Leaving the error to `when` therefore turns every errored watch into another indefinite spinner, on a list exactly as on a subject.

A watch that still holds **renderable** data is exempt from the error surface: showing the last known rows beats replacing them with an error panel mid-retry. The exemption is keyed on there being something worth preserving, not on `hasValue` alone — Riverpod retains the previous value across a failure, so a watch that emitted an empty list (or, on a subject, `null`) and *then* errored still reports `hasValue`. Gating on that bare flag would route the failure into the empty or missing surface: a broken read would be indistinguishable from an empty inbox, and a subject whose watch merely failed would claim it had been deleted and offer an escape premised on that. So `AsyncList` exempts only a non-empty list, and `AsyncSubject` only a non-null subject.

### Inbox row tap flow

Tapping an inbox row navigates to `/inbox/:id/clarify` (`InboxClarifyScreen`), a focused, full-screen clarification flow that:

1. Watches the Capture live via `captureProvider` (`CaptureDao.watchCapture`) and renders it through `AsyncSubject` (see *Async surfaces* below), so loading, an error and a Capture hard-deleted on another device are three distinguishable states. Its non-person tag hints (`CaptureDao.tagHintsForCapture`), which seed the new Outcome's tags, stay a one-shot read: the screen renders no tag pickers, so there is nothing on screen for a stream to keep in step.
...
The shared UI primitives (`ClarifyFieldLabel`, `ClarifyEnergyPicker`, `ClarifyEstimateChip`, `ClarifyDestinationButton`) live in `app/lib/widgets/clarify_shared_widgets.dart`. `InboxClarifyScreen` uses the field primitives directly, plus `ClarifyDestinationButton` for Skip; its routing buttons come from `ProcessToHandlers`. The planning wizard's `InboxClarificationStep` and the periodic-review wizard's `ZeroInboxStep` both delegate to the shared `ClarifyStep` widget (`app/lib/widgets/ceremony/clarify_step.dart`) — literally the same class in both ceremonies. `ClarifyStep` wraps `ClarifyCard` (`app/lib/widgets/clarify_card.dart`) with inline loading/empty/completion branching, accepting ceremony-specific state (nav cursor, routings, callbacks) as constructor arguments with no hard-coded provider dependency. Among those callbacks is `onSubjectMissing`, fired when the user escapes a card whose Capture was hard-deleted underneath them: DPR wires it to `skipInboxItem`, WR to `advanceInbox` — the cursor advances *without* recording a routing, because there is no verdict to record for a row that no longer exists. It is deliberately not routed through `onAfterRoute`, whose only "no verdict" action (`ProcessAction.keep`) has a null `toRoutingKind()` that both ceremony callers early-return on, so the cursor would never move. The completion view is the canonical `_InboxCleared` widget hard-coded inside `ClarifyStep` — the "Inbox is clear" frame is identical across ceremonies and is not parameterised. `ClarifyCard` also drives the periodic-review `Re-clarify…` sub-flow surfaced from the Waiting For / Next Actions / Someday-Maybe steps; that pushed route is the one clarify host with no footer, so the card's own missing-state CTA (which pops it) is the only escape *inside* the card. It is not the only way out: the AppBar arrow and system back also close the route, popping with no result. Both land on the same outcome — when the subject is gone, the outer widget bubbles `keep` rather than routing the null through its existence-checked `_keep()`, which would return early and strand the cursor on the deleted item.
...
The card's Tags section (above Energy) edits the **categorisation axes** only: a single project tag via `ProjectPickerWidget` and multiple context tags via `ContextTagPickerWidget` — the same pickers the task-detail screen uses. On an Outcome card both autosave through `TaskDetailNotifier` (`assignProject`/`clearProject`/`assignContextTag`/`removeContextTag`), which mutate only the `todo_tags` join table; on a Capture card the same edits go through `CaptureDao` instead (`assignProjectHint`/`clearProjectHint`/`assignTagHint`/`removeTagHint`), persisting as hints in `capture_tags` because no Outcome exists yet to own a join row. Person tags are deliberately absent from the card: delegation is assigned exclusively through the Waiting For routing button's `PersonTagPickerSheet`, which writes intent + person tags atomically. Because context/project edits never touch `todos.intent` or person-tag join rows, the intent ⊥ delegate orthogonality invariant holds structurally — attaching a project tag to a `maybe` task leaves it `maybe`. Unlike the person-tag methods, these edits do not stamp `last_clarified_at`; the clarified moment is bound to the delegate axis, not categorisation.

On a Capture the same orthogonality has to be defended on the *hint* side too. `CaptureDao.watchTagHints` returns every row joined through `capture_tags` without filtering on type, and person-typed hints genuinely occur — the Nirvana migration copies a delegated inbox todo's `todo_tags` wholesale into `capture_tags`. `ClarifyCard` therefore excludes person hints when it seeds its draft tag set, not only when it builds the `ClarifyDraft`: the build-time screen compares against the hint list *currently* emitted, so a person hint that was present when the draft seeded but has since disappeared from the stream would no longer be recognised and would ride onto the new Outcome — producing a row routed to Next that carries a delegate it was never delegated to.
...
- **Task deleted mid-sprint**: the screen renders its subject through `AsyncSubject` with a `missingBuilder` that tears the sprint down exactly as **Stop** does — cancels the notification refresh timer, cancels the persistent focus notification, `stopSprint()`, `endFocus()` — then bounces to `/focus` with an explanatory SnackBar. Focus is a sprint on one task, so a panel asking the user to acknowledge a task that no longer exists would be worse than the redirect; but the redirect must say why (the previous bounce was silent) *and* must actually end the sprint. Navigating alone would leave focus mode active on a deleted row, the sprint timer ticking toward an end-notification for a task that no longer exists, the status-bar notification deep-linking back into the dead sprint, and the TimeLog open.
