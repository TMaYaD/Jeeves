...
**The cursor is retired (story 9, issue #479, ADR-0022).** No code path writes `todos.next_action_text`, and none has read it since story 3. The column stays declared and keeps replicating for old-client compatibility, but the `actions` table is the only grain. The legacy one-field surfaces survive as thin entry points that drive `ActionDao` and nothing else — `TodoDao.setNextActionText` (blank routes to `clearCurrentAction`), `TodoDao.setNextActionTextIfActionless` (issue #501 — the **atomic actionless-mirror**: reads the `current` Action and writes the Action inside **one** transaction, so a `current` Action landed by sync cannot slip between the check and the write; it takes `setNextActionText`'s write path verbatim through the shared `_applySetNextActionText` body when Actionless, and is a pure no-op — no write, no stamp, no notify — when a `current` Action already exists; blank text is a caller error), `TodoDao.applyRouting`'s `nextAction` / `waitingFor` arms (blank → clear, absent → no-op; the other arms write no Action), and `TodoDao.deleteOutcome` (carve-undo cascades the Outcome's Action rows explicitly — the local view enforces no FK cascade). These keep their names and their exact atomicity and notify semantics; only the storage moved.

There is consequently **no correspondence between cursor and Action to maintain**, and the old *blank cursor ⟺ no `current` row* invariant is gone. Nothing should restore it. Each role transition used to clear or rewrite the cursor to keep the startup sweep from undoing it; none does now, because the row each transition leaves behind — `planned` after a demote, `done` after a completion, `superseded` after an abandon — is itself what stops the adoption pass minting a replacement. `_applySetNextActionText` still writes `todos`, but only the `last_clarified_at` / `updated_at` stamp: these surfaces stamp even when the Action write is a no-op, because re-submitting the identical phrase is still a clarifying micro-act.
...
A peer that predates the Actions table writes the cursor alone, so the store can hold the two describing different things. Nothing repairs it, deliberately: the sweep never adopts over existing rows and no longer retires on a blank cursor. Read the Action row, always. ADR-0022 records why losing such a client's edit is preferred to letting it clobber Action-grain history, and why downgrading across this change is hazardous.

**Metadata is Action-grain (story 7, issue #477).** `energy_level` and `time_estimate` now describe the *action of doing*, so a replacement Action never inherits the effort metadata of the one it replaced. Four rules govern the coexistence of the Action rows and the still-present Outcome columns. Unlike `next_action_text` these columns are **not** retired: D2 reads them, so the mirror is load-bearing rather than compatibility scaffolding, and it deliberately outlives the cursor. Retiring it needs a D2 read-rule redesign of its own.

- **D1 — mirror invariant.** `TodoDao.updateFields` — the one live writer of these fields on an existing Outcome — is transactional and writes both grains: it keeps `todos.energy_level` / `time_estimate` mirrored *and*, when a `current` Action exists, edits it through `ActionDao.applyEditAction` in the same transaction with the same timestamp (a `clear*` flag nulls a column on both sides). So the two sides never drift.
...
- **`adoptCursorsWithoutActions` — cursor-dependent, mint-only.** Mints one `current` Action, at the deterministic backfill id `backfillActionIdFor(todo_id)` (ADR-0019, so independent devices converge on one row rather than minting divergent random-id rows), for a **live** Outcome that carries a non-blank `next_action_text` **and has no `actions` rows at all**. The `NOT EXISTS` guard spans every role, and it is the whole safety property: the pass can never resurrect an abandoned Action, revive a completed one, or overwrite an edited one. Live means `done_at IS NULL`, and nothing more: completion terminates the current Action (story 4), so minting onto a completed Outcome would manufacture a state the app never writes. Every other lifecycle state is kept, because adoption is a one-shot rescue that dies with the cursor columns — skipping an Outcome loses its next-action text for good. Someday/Maybe and unclarified Outcomes qualify because `applyRouting`'s `maybe` arm leaves both the cursor and existing Action rows alone. Trashed Outcomes qualify for the sharper reason that `applyRouting`'s `trash` arm does not terminate Actions either; Trash has no expiry and no purge affordance, restore is one tap, and every GTD list gates on `intent IN ('next', 'maybe')`, so the minted row preserves the text without changing any list, count or badge until the user restores the Outcome. Waiting For needs no clause: it is `intent = 'next'` plus a `Tag(type='person')`, not a separate intent.

Because the mint id is a client-side uuid5, the pass cannot express "id not already taken" in its SELECT — its `NOT EXISTS` guard is over `outcome_id`, and a legacy row carrying a NULL `outcome_id` satisfies that guard while still occupying the primary key. So the pass reads back which candidate ids are present and skips those Outcomes; a bare insert would instead abort the startup write transaction on a unique violation. Skipping is the correct resolution, not a fallback: the id is a pure function of the Outcome, so whatever sits at that key already *is* its adoption, and overwriting it would be the cursor-authoritative behaviour the sweep deleted. The pass exists only to adopt an Outcome created by a client that predates the Actions table, and dies with the columns.
