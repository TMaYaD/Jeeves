...
Stubs still left for the siblings: Workspace genesis, Grants, roles, `epoch_floor`, and the remaining control types (#549 — `unsupported_control_type` is their landing slot, the recorded storage-DoS residual of an unchained member posting content is theirs to close, and it additionally owes **closing live signal sockets on revocation**, since a socket is authorised once at the handshake and never re-checked); the remaining collections and ADR-0011's Conflict Strategy registry (#550); per-author chain verification, quarantine heal and integrity alarms (#551, the eventual consumer of `SignalListener.syncFailures`); enrolment and passphrase *screens* (#553 — the logic and its service API are here); KeyWraps and AEAD (#554 — `master_wrap_key` is escrowed and read by nothing); compaction/prune (#555; `ops.compacted_by` exists and is unused).
