...
**The `aead_v1` suite and the key plane are half-built (#554).** Both codecs serve suite `0x01` — the body is XChaCha20-Poly1305 over the *same* framed body `plaintext_v1` would carry, with the literal 158 header bytes as AAD, so the padding rules run on the decrypted plaintext through the same function and the suite, epoch and nonce are bound with no second binding. Exactly one rule is suite-conditional (a `0x01` body is a size class plus the 16-byte tag), and exactly one pair is forbidden outright: an `op_class = 2` op under `0x01` is `encrypted_control_op`, because the server materialises control payloads and holds no key. Both KeyWrap flavours, the `keywrap_digest`, and the `rotate` control op are defined and vector-pinned (ADR-0037). Server-side the key plane is complete: alembic `0035` adds `keywraps` and `workspace_epochs`, `PUT /w/{w}/keywraps` accepts a whole wrap set only when it hashes to the digest the signed `rotate` already committed to, `GET /w/{w}/keywraps/me` and `GET /w/{w}/epoch-keys` serve wraps the server cannot open, and a content POST more than one epoch behind `MAX(workspace_epochs.epoch)` is refused `key_epoch_stale`. What is **not** built is the client half: no Device holds a Workspace key, mints a wrap, runs the rotation ceremony, or emits an `aead_v1` op, so `epoch_floor` is still raise-only machinery with nothing raising it and the *orphaned grant* predicate still never fires. Encryption turning on is a per-Workspace owner ceremony rather than a deploy, which is what lets the server ship this far ahead of the client with nothing changing for a `plaintext_v1` device.

Stubs still left for the siblings: `member_key_rotate` and the remaining control types (rotating a *Member's* own signing or KEX key, as opposed to the Workspace content key `rotate` moves — `unsupported_control_type` is their landing slot); alarm **UI** — `syncStatusProvider` reads `watchSyncHealth()` and collapses an unresolved alarm or a quarantined op into one error state, which is honest but says nothing about *what*; `watchIntegrityAlarms()` and `watchQuarantineCount()` are the seam a surface that explains it would read, and no widget does; cross-author fork detection, which needs `observed_head` (still zeroed) and author heartbeats, so author silence stays indistinguishable from offline; the passphrase-*change* screen, since `changePassphrase` still has no caller; and compaction/prune (#555; `ops.compacted_by` exists and is unused). The server-side mirrored tables and their REST routes are gone — #556 retired them.
