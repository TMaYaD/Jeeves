...
Stubs still left for the siblings: Workspace genesis, Grants, roles, `epoch_floor`, and the remaining control types (#549 — `unsupported_control_type` is their landing slot, and the recorded storage-DoS residual of an unchained member posting content is theirs to close); the remaining collections and ADR-0011's Conflict Strategy registry (#550); per-author chain verification and quarantine heal (#551); the realtime signal (#552); enrolment and passphrase *screens* (#553 — the logic and its service API are here); KeyWraps and AEAD (#554 — `master_wrap_key` is escrowed and read by nothing); compaction/prune (#555; `ops.compacted_by` exists and is unused).
