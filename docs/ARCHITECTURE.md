...
- **`backend/app/sync/`** — `ops` (append-only per Workspace, `seq BIGSERIAL` as a transport cursor only) and `members` (public keys, no authority), plus three endpoints: `POST /members`, `POST /w/{w}/ops` (batch, idempotent by author-namespaced op id, `author_seq` contiguity enforced), `GET /w/{w}/ops?since=&limit=`. Alembic `0031` adds both tables; neither joins the PowerSync publication. The server parses the 158-byte header for its index columns and its content-blind authz, and never a body.
