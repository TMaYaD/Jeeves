...
**A `planned` Action's metadata is Action-local and is never mirrored.** The planned-queue effort pickers (`_PlanActionSheet`, issue #477) drive `ActionDao.addPlannedAction` / `editAction`, both of which write the `actions` row and nothing else. That is required, not an omission: D2's COALESCE reads the Outcome columns as *the current Action's* values, so a planned row reaching them would be read back as the effort of an Action the user has not started — and a `planned` row is not even engageable (ADR-0004). The mirror is re-established at exactly the moment the row becomes current, by `_promoteRow` (and by `applySupersedeCurrentAction` on the replace path), which is why the feature needs no new bookkeeping to preserve D1–D4. The corollary is that `editAction` must not be pointed at a `current` row: it would move the Action without the mirror, and the columns would keep the old values until something else rewrote them. `TodoDao.updateFields` is the current-grain writer; nothing in the UI routes a `current` row through `editAction`.
