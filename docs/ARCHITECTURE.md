...
#### The `actions` table (issue #471 story 1, issue #472 story 2, ADR-0001)
...
- **Outcome routing** (review surfaces — re-clarifying an Outcome that already exists) — `clarifyToOutcome`, `completeOutcome`, `stampClarified`, `updateFields`, plus the `exists` / `getPersonTagIds` reads the flows need to guard their writes. `DaoClarificationService` delegates these 1:1 to the `TodoDao` (and, for the capture-existence read, `CaptureDao`) methods below. Inbox flows do **not** use this family: clarifying a Capture goes through Capture clarification below.
