...
- **Outcome routing** (review surfaces, and today's Inbox flows) — `clarifyToOutcome`, `promoteCaptureToOutcome`, `completeOutcome`, `stampClarified`, `updateFields`, plus the `exists` / `getPersonTagIds` reads the flows need to guard their writes. `DaoClarificationService` delegates these 1:1 to the `TodoDao` (and, for the capture-existence read, `CaptureDao`) methods below. `promoteCaptureToOutcome` is the legacy in-place 1:1 promote — it delegates to `TodoDao.processInboxItem`, which flips `clarified = true` on an unclarified `todos` row and stamps `last_clarified_at`.
