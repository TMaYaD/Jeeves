...
- **`app/lib/sync/`** — the v1 envelope codec, HLC, stub member identity, a client-owned Drift store (`op_log`, `outbox`, `author_state`, `sync_cursors`, `quarantined_ops`, reduced state), the field-grain reducer with a collection registry, and `SyncClient` over an abstract `SyncTransport`. Plus the four pieces that carry the domain across it, described below: the capture seam, the per-collection codecs, the merge-strategy registry, and the domain projector.
...
Stubs left for the remaining slices: the Root chain and member-scoped JWTs (#548), Workspace genesis and Grants (#549) — a User reaches exactly one implicit Workspace, `uuid5` of the user id — chain verification and integrity alarms (#551), the realtime signal (#552), the live-app cutover (#553), E2EE (#554), and compaction/prune (#555; `ops.compacted_by` exists and is unused). PowerSync removal and the dead-column drop ride #556.
