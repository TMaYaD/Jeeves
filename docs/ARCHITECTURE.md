...
3. In 1-1 mode, renders the canonical `ProcessToHandlers` action bar for the routing verdict (Next Action / Waiting For / Someday / Done / Discard), then saves the Capture's text edits and pops via `onAfterRoute`. The bar owns every routing write, so this screen cannot drift from the ceremony clarify surfaces. In n-m mode that bar is replaced by `CaptureOutcomesField` (see "The n-m clarify surface" below).
...
In **1-1 mode** — the shipped path, unchanged — `InboxClarifyScreen` and `ClarifyCard.forCapture` render `ProcessToHandlers`, and each routing verdict calls `clarifyCaptureToOutcome` (create, link, stamp) or `discardCapture`.

In **n-m mode** both surfaces render `CaptureOutcomesField` (`app/lib/widgets/capture_outcomes_field.dart`) in that slot instead:

- **One field does both gestures.** Typing runs `TodoDao.searchOutcomesByTitle` (case-insensitive substring over live Outcomes — trashed and achieved ones are excluded, LIKE wildcards escaped). Picking a match calls `mergeIntoOutcome`; picking the always-present `Create "…"` row calls `carveOutcome`. There are no separate "new" / "link existing" affordances, because split and merge are the same gesture. An Outcome the Capture already claims is dropped from the suggestions.
- **Neither write stamps `clarified_at`.** That is the whole difference from 1-1 mode: the Capture stays in the Inbox while Outcomes accumulate against it, and carves accumulate rather than overwriting. A carved Outcome is created by `TodoDao.insertOutcome`, so it lands with the default `intent = 'next'`; refining that is the Outcome's own re-clarification, not the Capture's.
- **The footer is one button whose label swaps on the linked count** — "Done with this Capture" at ≥ 1 (`completeCaptureClarification`, stamp only, every link kept), "Discard Capture" at 0 (`discardCapture`). Both stamp; there is no confirmation dialog.
- **Unlink cuts by provenance.** The surface tracks which Outcome ids it carved this session; unlinking one of those passes `deleteCarved: true` and the Outcome is deleted, while unlinking anything else only detaches. The delete stays gated on no *other* Capture claiming the Outcome, so one Capture's undo can never destroy another's merged work. Session-scoped by necessity: once written, a carved and a merged Outcome are the same `todos` row.
- **Provenance is on the row.** `CaptureDao.watchCarvedOutcomes` (via `carvedOutcomesProvider`) returns each linked Outcome with the number of Captures claiming it; more than one renders a "from N Captures" chip.

The ceremonies reach it through `ClarifyCard`'s `onCaptureCompleted` callback, wired by `ClarifyStep.onAfterComplete`. It is deliberately separate from `onAfterRoute`: the n-m verdict picks no destination, so there is no `ProcessAction` to report and no routing for the ceremony to record for its "previously selected" affordance.
