...
- **Sync Engine:** the minimal sync server's content-blind op log — signed ops over per-Workspace logs, replicated device to device (ADR-0026). PowerSync remains on device as the local store engine only, disconnected; its removal is #556.
...
sign-up, so `_handleMigration` reassigns every local row to the new account and
enqueues all of them in `ps_crud`; a connected engine would upload the whole
migrated store into the new account's legacy tables, a second live sync path into
tables #556 deletes. Two accepted consequences: `ps_crud` grows with nothing
draining it (disk on one device, bounded by the confidence window, gone with the
file), and a signed-in but **un-enrolled** device no longer replicates at all —
which is what the flip is, and does not touch local function.

The rest of this section describes that machinery as it stands. It is the shape the
rows on a device that ran on the legacy path still have, which is what the initial
upload reads:
...
**Reconciliation sweep:** `reconcileActionsAtStartup` (`migration_service.dart`) runs at startup from `powerSyncInstanceProvider`, right after `migrateLocalInboxToCaptures` and before any watcher exists (so it needs no view-notify). It writes raw SQL and therefore authors **no op**, which is deliberate and justified in its own docstring: it runs before sign-in and long before the capture seam is bound, so wrapping it would author nothing anyway, and every device applies the *same* deterministic winner rule independently — so the divergence is idempotent churn on a column no read trusts, not lost data. It is **one cursor-free pass**, and it reads `actions` and nothing else:
...
**This is the production sync path** (#591, [ADR-0034](./adr/0034-sync-starts-at-enrolment.md)). A self-contained sync stack carries the whole domain — the content-blind op log that ADR-0026 replaces PowerSync with, plus ADR-0028's trust root — and the app writes and reads through it.

`app/lib/providers/sync_stack_provider.dart` opens its own `jeeves_sync.sqlite` (a Drift-native store, deliberately not PowerSync's file), reads this Device's identity out of the platform key store, puts `HttpUserTransport` on `ApiService.sessionDio` so the User-credential sync routes ride the same session and the same 401 refresh as every other call, and attaches the `DomainProjector` over the `GtdDatabase` to every client the stack builds. Assembling the stack does not start syncing; `providers/sync_lifecycle_provider.dart` does.

**Sync starts at enrolment.** `SyncLifecycle.activate` (`app/lib/sync/sync_lifecycle.dart`) is one closure with two callers — the enrolment outcome, and `main.dart`'s eager watch at every launch — and it runs six steps in a load-bearing order: derive enrolment state from the store with no network (`sync/enrolment_state.dart`; not enrolled or half-founded and nothing happens at all); re-mint the member credential by proof-of-possession over the stored keys if none is in hand, since the credential is memory-only by design and a relaunch therefore has none; bind the capture seam; sync both Workspace clients, because the initial upload's diff skip is only meaningful against a pulled log; run the **initial upload** if this account's marker is unset; start one `SignalListener` per Workspace and a debounced outbox flusher, without which a DAO-authored op would sit in the outbox until the next cold start.
...
Stubs still left for the siblings: `rotate` and the remaining control types (#554 — `unsupported_control_type` is their landing slot; `epoch_floor` is persisted, monotone and consulted on authoring, but nothing raises it in anger yet, and the *orphaned grant* predicate is defined against epoch 0 and never fires); alarm **UI** — `syncStatusProvider` reads `watchSyncHealth()` and collapses an unresolved alarm or a quarantined op into one error state, which is honest but says nothing about *what*; `watchIntegrityAlarms()` and `watchQuarantineCount()` are the seam a surface that explains it would read, and no widget does; cross-author fork detection, which needs `observed_head` (still zeroed) and author heartbeats, so author silence stays indistinguishable from offline; PowerSync's *retirement* — its disconnection is done (#591), but the engine, its store file, the mirrored tables and the REST routes all go with #556 — plus the passphrase-*change* screen, since `changePassphrase` still has no caller, and the real onboarding flow that replaces the throwaway enrolment-ceremony surface (which is what calls `SyncLifecycle.activate` today; the reseed screen survives beside it as optional verification, described in docs/TESTING.md); KeyWraps and AEAD (#554 — `master_wrap_key` is escrowed and read by nothing); and compaction/prune (#555; `ops.compacted_by` exists and is unused). PowerSync removal and the dead-column drop ride #556.
...
**Value encodings are protocol surface.** Drift `dateTime` columns encode as an ISO-8601 UTC instant truncated (never rounded) to exactly three fractional digits with a trailing `Z`; TEXT timestamp columns pass through byte-for-byte as opaque strings; text, int, bool and null pass through as JSON natives. The initial upload (`app/lib/sync/initial_upload_plan.dart`) reuses these codecs, so it cannot emit anything else. The *reading* half — `parseTimestampUtcMs`, the tolerant grammar frozen in `spec/converge_verify/` — lives beside them in `collection_codecs.dart`, because a store written before the encoding existed holds microsecond- and offset-bearing strings that must map onto the truncated wire value rather than being refused.
...
**`SyncHealth`** (`app/lib/sync/sync_health.dart`) is what the sync indicator reads: queue depth, unresolved integrity alarms and their kinds, quarantine count, and a `lastSyncedAt` stamped on **pull completion, independent of flush state**. `clean` is a derived getter, never a stored status — a wedged outbox shows through `pendingOpCount`. `syncStatusProvider` now maps it (both Workspaces' health, so a wedged preferences queue is a wedged device) through the pure `syncStatusFor` table, and no longer reads PowerSync's status stream: a permanently idle engine would map to `synced`, which is a green light on a device that is not syncing at all. The dead-letter watch went with it — nothing uploads through the connector, so no new dead letter can be recorded, and the table rides #556.
