...
`todos.next_action_text` is **dropped**. ADR-0022 retired it by abandonment and argued explicitly against a drop; this reverses that. The client half ships first — the column leaves `tables.dart`, the generated PowerSync schema, `TodoDao.todoProjectionSql`, and the Drift v26 backfill that was its last consumer; a `from < 28` migration drops it from any real table. A separate backend PR drops it from Postgres, atomically across the Alembic migration, the SQLAlchemy model, all three Pydantic schemas and the route, only after the owner confirms the rollout condition below.
...
## The ordering constraint is real, but not the one ADR-0022 assumed

ADR-0022 reasoned that a server-side drop would make the column vanish from every client at once. It does not: **PowerSync does not replicate DDL.** Rows already in bucket storage keep their value, and the column decays row-by-row as each row is next updated. A post-#479, pre-drop client is therefore fine — `Todo.nextActionText` silently goes null, nothing reads it, and its uploads still 201 because Pydantic's default `extra='ignore'` drops the retired field.

The build that must not be running is not "pre-drop" but **pre-#479**. Its startup sweep still carries Pass B: it retires every `current` Action whose Outcome has a blank cursor. As the cursor decays to NULL that build destroys the Action grain on the device **and syncs the deletions to every other device.** Wiping the offending device does not repair it — the damage has already replicated. ADR-0022 § Version skew already recorded that those binaries are unreachable by release ordering; this is the concrete consequence, and it is why the rollout is hand-sequenced rather than merged-and-forgotten.

The catastrophic mode is on the backend side: if the Alembic drop lands while the route still passes `next_action_text=` to the `Todo(...)` constructor, every todo POST 500s, PowerSync classifies 5xx as retryable, and the upload queue wedges permanently on every device. Hence the backend PR must be atomic.
