...
**Keep PowerSync over an opaque-row store.** Rejected: retains the operational weight (replication slot, Rust service) while destroying PowerSync's one benefit — that the synced table *is* the local table — since a decrypt/project step would be needed anyway.
