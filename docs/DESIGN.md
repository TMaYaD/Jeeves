...
Capture is reachable from **every** working surface, not just the Inbox: a thought that occurs anywhere in the app must be recordable on the spot (GTD's zero-friction capture tenet). A persistent **capture FAB** (`CaptureFab`, `app/lib/widgets/capture/capture_fab.dart`) renders on every shell route (Now, Next Actions, Waiting For, Maybe, Done, Trash), the task-detail screen, all three ceremony wizards and their completion screens, Search, Active Focus, Clarify, Settings and Import. It is **suppressed on the Inbox** — the `QuickAddBar` pill already owns capture there, and a second affordance would be redundant. It is also absent from the auth screens (pre-identity) and the Evening Shutdown Close Day screen (a full-screen commit-and-exit animation where mid-exit capture is incoherent).

The FAB is a **genuine circle** — the surviving circular case under the radii ruling above — and keeps one identity everywhere, the primary blue `#2563EB`, regardless of the host screen's accent, because capture is one concept app-wide. There is no single global overlay: a shell-scaffold FAB can't reach routes pushed above the shell, so the FAB is placed per-scaffold, with two leverage points doing the bulk — the `AppShell` scaffold (all seven shell routes at once) and the shared `Wizard` widget (all three ceremonies at once). In the wizard it is anchored to the bottom-right of the **content region** (inside a `Stack` over the `PageView`), so it sits above the step footer's single forward-affordance slot by construction, whatever the footer's height. Scroll views under the FAB carry ~96px of trailing clearance so their last content scrolls clear of it rather than sitting under it.

Tapping the FAB opens the **capture sheet** (`CaptureSheet`, `app/lib/widgets/capture/capture_sheet.dart`), a modal bottom sheet on the canonical 2/4/6 radii — 6px top corners (sheet surface), 4px input, 2px submit button (QuickAddBar's legacy `circular(999)` pill is **not** propagated). The input autofocuses so the user can type immediately, and the sheet **stays open on submit**: the field clears and refocuses so rapid multi-capture is one keystroke per thought, and an **inline confirmation row** ("Captured to Inbox", with a running session count on repeats) is the affordance that the Capture landed — there is no snackbar and no navigation away from the screen the user was on, honouring the no-toasts direction. Write errors surface as inline text in the sheet, never a toast. The write reuses the Inbox `QuickAddBar` path exactly (`inboxNotifierProvider.addCapture`): a raw **Capture**, `capture_source: 'manual'`, `clarified_at` NULL (ADR-0006) — indistinguishable from one typed on the Inbox. A Capture made mid-ceremony simply waits in the Inbox for the next clearing; it never shifts a ceremony's snapshot cursor (ADR-0009).
