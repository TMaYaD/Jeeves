...
- **`prepare-commit-msg` and the issue reference it appends**: the hook reads the current branch name and appends an issue reference (`(#605)`) to the commit subject, but its guiding rule is that the reference must be a *real* issue number or nothing at all — a wrong `(#NNN)` links the commit to an unrelated issue in the tracker permanently. It therefore extracts the number only from an **anchored, conventional branch shape**, never from any digit run in the name: a Jira-style `JVS-<number>` token, a generated worktree branch's leading `issue-<number>` (`issue-666/slug`, `issue-304-plan`), a PR branch's number right after the first slash (`fix/605-slug`, `feat/584-slug`), or a bare leading `<number>-<slug>`. This matters most for agents, who commit from `git worktree` branches whose names are generated: shapes like `review-655`, `coderabbit-review-proxy-3048c7` and `worktree-agent-<hex>` match none of the anchored forms and append nothing (the old first-digit-run-anywhere pattern turned them into `(#review-655)`, `(#proxy-3048)` and `(#0)`). The already-referenced guard matches the **whole `#<id>` token at a digit boundary**, not a bare substring, so it neither fires spuriously on a degenerate id nor is fooled by the id appearing inside a longer number in the body. A non-`message` `COMMIT_SOURCE` (merge, squash, template) is left untouched. Coverage lives in `.githooks/tests/test-prepare-commit-msg-hook.sh`, which drives the real hook through an actual `git commit` on a throwaway repo checked out on each branch shape.
