...
- **On the x86_64 builder, run tests through [`tools/builder_run.py`](../tools/builder_run.py) rather than `ssh`-ing in and calling `flutter test` by hand.** That Mac also runs a large agent fleet, and when it is oversubscribed a busy thread is granted as little as 8% of a core — wall clock then runs up to 12x ahead of progress, the guest's vCPUs starve with everything else, and the 60s budget fires on whichever test happens to be open. The tell is that the inflated tests are *arbitrary*: both concurrent isolates inflate by the same amount at the same instant, and guest load does not predict it. This is not the "do less work" case above — that one fails a test near its budget, this one fails a random one, so shrinking the suite cannot fix it. The wrapper takes an exclusive host lock so two builder runs queue instead of halving each other, waits for *every* host probe to report SANE before starting anything, and measures again at the end; it exits 0 pass, 1 failed, 3 unproven (the window fell mid-run — re-run, never read the red), 4 no sane window (nothing ran, so there is no verdict), 5 lock busy, 7 not configured. **It probes the host, never the guest** — measured in one window, the guest read 96% of a core (sane) while the host read 12% (void), because VirtualBox's vCPU threads hold a share a fresh host process does not. **Two resources are gated, because the host starves the guest two ways and each probe is blind to the other's.** [`tools/host_cpu_share.py`](../tools/host_cpu_share.py) asks whether a fresh process is being scheduled; [`tools/host_memory_pressure.py`](../tools/host_memory_pressure.py) asks whether the host is paging, which is what stops the guest for tens of seconds at a time — a spin loop over a resident working set never page-faults, so the CPU probe reads a swap storm as SANE, while a separately measured window had the host heavily loaded and swapping not at all, so neither probe subsumes the other. **Which machine to reach lives on the machine, not in here:** `~/.jeeves/builder.json`, shape in the wrapper's docstring. This repository is public, and the account, port-forward, key and checkout path describe one Mac rather than this project — as does any re-calibration of the probe thresholds, which the same file carries. Priority is not a lever — measured, `nice -20` changes nothing — and thermal throttling, Time Machine and Spotlight were each ruled out by measurement.
