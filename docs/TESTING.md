...
- **A test can pass on CI and time out on the agent host, and the answer is to do less work rather than allow more time.** The Pi runs this suite at roughly 7-9x the dev-Mac wall clock against Dart's default 30s per-test timeout — which nobody here chose — so a test comfortable at 7s locally sits near a minute there and fails where CI never looks. `convergence_test.dart`'s negative-control test hit exactly that, enrolling a fresh device per vector because `quarantined()` returns every row a device holds and a pristine receiver was the only way to read the set as "what this vector produced". Diffing the rows by id instead lets one receiver judge all eighteen: ~7s to ~1s on a dev Mac, and from timing out to 4s on the Pi — plus a stricter assertion, since each vector is now judged on the rows it actually added rather than on its predecessors' too. Prefer that shape. A scoped `Timeout.factor` only trades a hard failure for a hardware-calibrated threshold that drifts with load, and it rescues neither hang mode above — an isolate blocked on real I/O inside `FakeAsync` never lets any timeout fire.
