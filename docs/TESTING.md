...
- `harness/` — `FakeSyncServer` (the in-process contract double, with `connectAsUser` / `connectAsMember` mirroring the real credential split, plus `injectUnchecked` and `poisonRegistry` for playing a hostile server), `SimDevice` (store, identity, HLC, `goOffline()`, a lost-POST-response fault, and a real enrolment ceremony), `SimWorkspace` (N devices of one User), `AuthorFixture` (the twin of `backend/tests/sync/builders.py`).
