...
- **`todos.time_spent_minutes` is excluded from every cross-device comparison.** It is a dead cache the projector deliberately never writes (ADR-0030), so comparing it would compare local defaults.

- `harness/` — `FakeSyncServer` (the in-process contract double, plus `injectUnchecked` for playing a hostile server), `SimDevice` (both stores, identity, HLC, `goOffline()`, and a lost-POST-response fault), `SimWorkspace` (N devices of one User), `reduced_state.dart` (`canonicalReducedState`, the deterministic byte string convergence is asserted over, and `domainRows` for full-row table comparison).
