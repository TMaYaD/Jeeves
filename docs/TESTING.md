...
- **The issue reference, appended by two hooks over one shared contract**: the `(#N)` a commit subject carries is added by whichever hook first has a subject in hand. `prepare-commit-msg` owns the `-m`/`-F` path, where git supplies the message before any editor opens; `commit-msg` owns the **editor path**, where the subject exists only after the editor closes — the commit `prepare-commit-msg` structurally cannot reach, and the gap #679 closed. Both source `.githooks/lib/issue-reference.sh`, so the accepted branch shapes and the bare-digits output are **one contract, not two**. The number is extracted by **anchored** POSIX parameter expansion, never by scanning the branch name for digits, and what reaches the message is always **bare digits** — `(#123)`, never `(#JVS-123)` or `(#review-655)`. The accepted shapes are `<type>/<number>[-<slug>]`, `issue-<number>/<slug>`, `JVS-<number>[-<slug>]`, and a bare `<number>[-<slug>]`; anything else appends nothing, silently and with exit 0. Three rules in there look removable and are not: **a leading zero disqualifies** (this repo's migration and ADR numbers always carry one, GitHub issue numbers never do, so `fix/0028-ambiguous-parameter` must not become `#28` — note that `fix/alembic-0028-…` is stopped one step earlier, by the anchor, so it is not evidence for this rule), the `issue-*/*` branch of the `case` **must stay above** the `*/*` branch (swap them and `issue-458/…` silently yields nothing), and the **trailing-number shape is deliberately unsupported** (`feat/login-ui-94`), because the tail of a branch name is where versions, ADR numbers and dependabot bumps live. The accepted cost is that ad-hoc working branches (`review-655`, `addr-595`) append nothing where they sometimes appended the right thing; the remedy is conventional branch naming, and the trade is deliberate — a false positive is permanent tracker pollution, a false negative is silent and harmless. Two residuals are known and unguarded: `chore/2026-08-audit` would append `(#2026)`, and — on the `-m` (`prepare-commit-msg`) caller only — `git commit -m 'feat: x' -m '#605'` appends a second reference, because git cleans up that path with `whitespace`, which **keeps** comment lines while the hook strips them. That is a cleanup-**mode** mismatch, not a comment-character one, and it behaves identically under `#`, `core.commentChar` and `core.commentString`; the editor path git cleans up with `strip`, so the mismatch cannot arise there.

  Two guards keep a reference from being added twice. The **already-referenced** guard matches `#<number>` **as a token** — followed by a non-digit or end of line — not as a substring, so `605` inside `1605` no longer suppresses the append and the degenerate id `0` no longer matches almost any message. The **trailing-reference** guard (#679) skips a subject that already ends in *some* `(#N)`, even a different issue's: it is what stops `feat: work (#605)`, amended on `fix/999-b`, becoming `feat: work (#605) (#999)`, since the already-referenced guard only looks for *this* branch's number.

  **`commit-msg` reconstructs the `$COMMIT_SOURCE` git does not pass it, from intrinsic signals.** On the editor path git hands `commit-msg` only the file, so the cases where an append would be wrong are read from state instead of from a source string: **merge** — `MERGE_HEAD` exists; **squash** — `SQUASH_MSG` exists (`git merge --squash`, rebase squash); **autosquash** — the subject begins `fixup!`/`squash!`/`amend!` (checked in the lib); **rebase reword/edit** — **HEAD is detached**, so `git symbolic-ref` fails, the branch name is empty and no number is produced. That last one is an accident of git's rebase implementation, not a guard anyone wrote, and it is the whole reason a reword or an autosquash-combine does not acquire a spurious reference — so it is pinned by a test (reword an unreferenced commit while checked out on a numbered branch; the subject must stay unreferenced), because a future git that stopped detaching HEAD there would silently break it. A conventional merge subject (`chore: merge x`) passes the format validation and would otherwise gain a reference, so the `MERGE_HEAD` guard keeps it byte-identical; a non-conventional `Merge branch …` is rejected by that validation first, as it is today.

  **`commit-msg` validates the subject, not the whole file.** It greps the **first non-blank line** — the same line the reference logic treats as the subject — so a Conventional-Commits-shaped line in the *body* cannot rescue a non-conventional subject (`git commit -m 'wip' -m 'fix: x'` used to record `wip` as the subject, and even gave it a reference; the suite pins that it is now rejected). `amend!` joins `fixup!`/`squash!` in the accepted prefixes: git authors those subjects for `--fixup=amend:`/`--fixup=reword:`, and `append_issue_reference` leaves all three verbatim, so they must clear validation to reach it — the suite pins a hand-typed `amend!` (no conventional body to lean on) passing and staying untouched.

  **Quitting the editor without saving still aborts — via that validation, not an emptiness check.** `commit-msg` runs the format check **before** the reference logic and before git's own empty-message check, so on a comments-only file (the quit-to-cancel gesture) the subject it reads is one of git's own comment lines, which never matches the regex — it exits 1 with **zero commits**, under the default comment character, `core.commentChar` or `core.commentString`, and under `commit.verbose` where the diff lines are `+`/`-`/space-prefixed. The suite pins that across the full **six-cell matrix** — {default, `core.commentChar=';'`, `core.commentString='//'`} × {`commit.verbose` on, off} — asserting zero commits in every cell. `prepare-commit-msg` still guards on `$COMMIT_SOURCE = message` so it never writes into the pre-editor file (which would fill an otherwise-empty message and defeat the abort); the reference the editor path is owed is added later, by `commit-msg`, once a real subject exists.

  **The reference lands on the first *non-blank* line, not line 1.** On the `-m` path line 1 is always the subject, so `prepare-commit-msg`'s output is byte-for-byte what it always was. On the editor path a developer may leave a blank line above the subject, and git takes the first non-blank line as the subject; appending to line 1 there would strand the reference on the blank line and corrupt the recorded subject (#679). The suite pins a subject typed below a blank line.

  **`--fixup`/`--squash` subjects are git's own, and the hooks leave them byte-identical.** `git commit --fixup <sha>`, `--squash <sha>`, `--fixup=amend:<sha>` and `--fixup=reword:<sha>` build subjects git has authored as `fixup! <target>` / `squash! <target>` / `amend! <target>` (both `amend:` and `reword:` emit `amend!`), and `git rebase --autosquash` matches them against the target's subject **byte-for-byte**; appending `(#NNN)` breaks that match, so the fixup never squashes and survives the rebase as a standalone commit (#675). The lib therefore leaves any subject whose first non-blank line begins `fixup!`, `squash!` or `amend!` followed by a space verbatim — exactly the prefixes git's own autosquash recognises, and the only signal that distinguishes a git-authored fixup from a *hand-typed* one. The suite pins this with a **real `git commit --fixup`/`--squash` and a real `git rebase --autosquash`**, and builds the fixup **target reference-free on `main`** before checking out the numbered branch — were the target built on the numbered branch it would already carry `(#605)`, and the already-referenced guard would suppress the append even on an *unpatched* hook, so the case would pass against the bug and prove nothing. That is the exact invisibility the issue names: the defect hides whenever the target already carries the reference. Beyond the byte-identical subject, the case asserts the fixup **collapses into its target** — the commit count drops by one, no `fixup!`/`squash!` subject survives, and the target subject gains no reference during the (detached-HEAD) rebase.

  The already-referenced guard reads the message through a helper that drops comment lines **and everything from the scissors line down**: on the `-m … -e` path (and the editor path) git appends its status block and, under `commit.verbose`, the staged diff below the scissors un-commented, so without the trim a `#605` inside the diff counts as an existing reference and suppresses a legitimate append. That trim matches on the **`---- >8 ----` run alone, never on the comment prefix in front of it**, so it needs to know nothing about the comment character and holds under `#`, `core.commentChar`, `core.commentString` and `core.commentChar=auto` alike. Comment lines are dropped by **`git stripspace --strip-comments`**, git's own plumbing, which reads `core.commentChar` / `core.commentString` itself — nothing is interpolated into a pattern and no prefix has to be escaped (a hand-rolled `grep -v "^$prefix"` would need both, since git's candidate list contains `$`, the BRE end-of-line anchor, which would silently strip every blank line). The defect this closes needs no unusual message, only an unusual config: under `core.commentChar=';'` on a branch named `fix/605-#605`, git's own status line `; On branch fix/605-#605` sails through a `^#` filter, and the guard reads the `#605` in **git's** text as an existing reference. Two properties are deliberate: `core.commentChar=auto` is **not** resolved (`stripspace` falls back to `#`, so under `auto` the helper can disagree with the character `git commit` chose — exactly what a hard-coded `^#` did), and if `stripspace` exits non-zero the helper yields **no text**, so the guard finds no reference and the append proceeds — it feeds only the already-referenced guard, never the message written, so its worst case is a visible duplicate `(#605)` rather than the silently-missing reference #666 was filed for.

  **The lib is located by a candidate list, and a missing lib degrades to a non-blocking no-op.** Each hook tries `$0`'s directory, then `git rev-parse --git-path hooks`, then `--show-toplevel`/`.githooks` — because a `.git/hooks/* → ../../.githooks/*` symlink layout puts `$0` where `lib/` is not, and only the last candidate finds it there. The missing-lib guard sits **before** the `.` that sources it, because `.` is a POSIX special builtin whose failure aborts the script under `sh`/`dash` and would otherwise reject the commit; bailing before it means a missing lib forgoes only the reference. In `commit-msg` the validation runs first and needs no lib, so a missing lib can never disable the format check every push depends on. The suite pins both (a commit still lands with no reference when the lib is gone; validation still rejects a non-conventional subject) and the symlink layout (the reference is still appended, via the last candidate).

  Two suites cover this, both through a **real `git commit`** in a throwaway repo with `core.hooksPath` pointed at an isolated directory (the real `.githooks` would drag in `pre-commit`'s whole gauntlet). `.githooks/tests/test-prepare-commit-msg-hook.sh` installs `prepare-commit-msg` + the lib and stays scoped to the `-m`/`-F` path; `.githooks/tests/test-commit-msg-hook.sh` installs the **production pairing** — both hooks + the lib — and drives the editor path with a scripted **`GIT_EDITOR`** (one that quits without writing, ones that type a subject above or below a blank line, one that replaces the file). Every assertion is on the message git recorded (`git log -1`), never on hook stdout — a hook that exits 0 having done nothing and one that exits 0 having done the right thing are indistinguishable by status; the quit cases assert on the **commit count**, not just the exit status, since a hook that fabricates a message makes git succeed. The autosquash cases (#675) drive a real `git rebase --autosquash` and assert the fixup collapses. A harness trap is load-bearing: an "unreferenced" fixture is **never** seeded with `--no-verify` (which skips `commit-msg` but not `prepare-commit-msg`, so a `-m` commit still gets `(#N)`) — it is seeded on `main`, where the branch name yields no number. Both suites run each case under **every shell present among `sh`, `dash`, `bash` and `zsh`** by rewriting the copied hooks' shebangs (the lib is sourced, so it runs under the hook's interpreter and needs no rewrite); an interpreter not on `PATH` is `SKIP`ped rather than run, so a box without `dash` or `zsh` simply exercises fewer rows (an empty shell list is itself a failing tripwire). `dash` is not redundant with `sh`, because CI's `/bin/sh` is dash while macOS's is bash in POSIX mode.
