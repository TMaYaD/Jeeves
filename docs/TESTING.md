...
- **A test can pass on CI and time out on the agent host, and the first remedy is to do less work.** The Pi runs this suite at roughly 7-11x the dev-Mac wall clock, so a test comfortable in single digits locally sits in the twenties there and fails on the one machine CI never looks at. Two were fixed by shrinking the work, not the assertion: `convergence_test.dart`'s negative-control loop enrolled a fresh device per vector purely so `quarantined()` could be read as "what this vector produced", and diffing the rows by id lets one receiver judge all eighteen (~7s to ~1s on a Mac, timing out to 4s on the Pi, and a *stricter* assertion); `chain_verifier_test.dart`'s deep-reorder scan dropped from 200 ops to 50, since `occurrenceCount` is 1 for a flat scan and N-1 for a nested one at every N, so the larger number bought reassurance rather than detection.
- **The per-test budget is 60s, set once in [`app/dart_test.yaml`](../app/dart_test.yaml), not Dart's 30s default.** It is a shade over twice the measured worst case on the slowest host, and still inside the minute that keeps a hang attributable. Deliberately one number for the suite rather than per-test `Timeout.factor` exceptions, which drift upward with every slower host and hide which tests are near the line — reach for the paragraph above instead. Neither the budget nor an exception rescues the two hang modes above: an isolate blocked on real I/O inside `FakeAsync` never lets any timeout fire. To see where tests actually stand, run the suite with `--reporter json` on the target host and diff `testStart`/`testDone` times.
