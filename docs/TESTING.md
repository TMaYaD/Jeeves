...
## Reseed (cutover tooling, #553 Phase 2)

The step the cutover rests on: author the phone's legacy PowerSync-side store onto the op-log spine as signed ops, then reduce the server log **from zero** on a throwaway stack and compare it with the source. If those agree, the Phase-3 flip changes transport, not data. **#556 removes the surface** (`app/lib/cutover/reseed/`, `app/test/cutover/reseed*`, the `/settings/reseed` route and the Settings entry, all marked) but not the machinery underneath it — `app/lib/sync/` and `providers/sync_stack_provider.dart` are permanent.

**How it works.** Three parts, one shared transform:

- `reseed_plan.dart` is pure: legacy rows in, planned ops out. Per-collection field encoding through the shipped codecs (`dateTime` columns normalised by the same timestamp grammar converge-verify uses, TEXT timestamp columns passed through byte for byte, `user_id` **stamped** with the enrolled account), junction and `user_preferences` ids rewritten to their derivations, and ADR-0025's auto-resolve. Both the uploader and the verifier read this one plan, so the comparison cannot degenerate into two normalisers agreeing with each other.
- `reseed_uploader.dart` authors it through `SyncClient.capture` — the production path, so the codecs, the receive-codec round trip, the author-side guards (#573, ADR-0033), the HLC, the signature and the outbox are all the real ones — and flushes on a cadence.
