...
- **On the x86_64 builder, check the host's CPU share before believing a timeout.** That Mac also runs a large agent fleet, and when it is oversubscribed a busy thread is granted as little as 8% of a core — wall clock then runs up to 12x ahead of progress, the guest's vCPUs starve with everything else, and the 60s budget fires on whichever test happens to be open. The tell is that the inflated tests are *arbitrary*: both concurrent isolates inflate by the same amount at the same instant, and guest load does not predict it. This is not the "do less work" case above — that one fails a test near its budget, this one fails a random one, so shrinking the suite cannot fix it. Run [`tools/host_cpu_share.py`](../tools/host_cpu_share.py) **on the host, not in the guest** — measured in one window, the guest read 96% of a core (sane) while the host read 12% (void), because VirtualBox's vCPU threads hold a share a fresh host process does not. It exits 0 sane, 1 degraded, 2 void, and a red run from a degraded window is unproven rather than failing. Priority is not a lever — measured, `nice -20` changes nothing — and thermal throttling, Time Machine and Spotlight were each ruled out by measurement. The guest runs on 2 of the host's 4 cores, leaving two for the host and for the emulation threads the guest depends on.
