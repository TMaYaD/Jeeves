...
- **A widget test that "hangs" is usually a pending timer, not a slow test.** When a test ends with a `Timer` still live, the binding's teardown assert fires *after* the test body — so the real assertion failure is swallowed and the run stalls (then takes the whole `flutter test` process down with `Bad state: Cannot close sink while adding stream`). Two common sources: a `SnackBar`, which holds a ~4s display timer, and any `Timer.periodic` owned by a **provider** rather than a widget (e.g. `SprintTimerNotifier`'s ticker), which outlives the widget tree entirely. Drain a SnackBar by pumping past its duration *before* the assertions — sample what you need into a local first, since an assertion that throws will skip the drain and reintroduce the hang. For a provider-owned ticker, override the notifier; the timer's own behaviour belongs in that provider's test, not in a screen test.
- **Subject-bound screens are driven by feeding a `StreamController`, not by deleting rows.** `AsyncSubject` screens watch a `watchSingleOrNull()`-backed provider, so overriding it with a controller and emitting `null` reproduces a synced hard-delete exactly, and `addError` reproduces a failed watch. Prefer this to a live drift `watch()` — see the next bullet for why one reached from a widget test hangs.
