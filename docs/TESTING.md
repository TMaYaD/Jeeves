...
- `harness/` — `FakeSyncServer` (the in-process contract double, plus `injectUnchecked` for playing a hostile server and a Dart mirror of the signal hub), `SimDevice` (store, identity, HLC, `SignalListener`, `goOffline()`, `goSilent()` for a half-open socket, a gated/failing pull, and a lost-POST-response fault), `SimWorkspace` (N devices of one User), `SimTimers` (a manually advanced timer wheel), `signal_probe.dart` (`pumpEvents`, `PokeRecorder`).
- `fake_sync_server_contract_test.dart` — the twin of `backend/tests/sync/test_ops_routes.py` and `test_signal_socket.py`, case-for-case under the same names. **Keep them in step:** a convergence test is only evidence about the real system if the double behaves like the real server, and a missing twin is how that stops being true. The file's header lists the handful of backend cases that deliberately have none, and why.
