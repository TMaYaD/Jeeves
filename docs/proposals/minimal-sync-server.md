...
**The control plane lives in the log.** Grant, Revoke, Rotate, MemberRegister, MemberKeyRotate are signed control ops chained to Root; server tables are a materialised index for its own authz, authoritative for nobody. Clients pin `root_pk` at first escrow unwrap (TOFU against the passphrase, not the server) and refuse Members not chained to it — the client's member directory is **chain-gated**, learning a key only from a MemberRegister it verified itself, so the `GET /w/{w}/members` registry is a bootstrap hint that verification never reads. Clients hold a monotone per-Workspace `epoch_floor`; a suppressed rotation is therefore a detectable gap, not a silent downgrade. **Control ops are never compacted.**

Control ops additionally chain **across authors** (F2): each carries, in its payload, `prev_control_hash` — SHA-256 over the **unframed payload bytes** of the latest control op its author had observed for that Workspace, not over the envelope and not over any re-serialization. Hashing payload bytes keeps the link computable from the unencrypted payload alone and survives any future envelope-level re-framing; the price is that a link points at bare bytes with no context, so **every control payload must be self-identifying — the `type` field is mandatory in every control type, for ever.** An all-zero `prev_control_hash` is valid only while the receiver has applied no control ops; arriving later it is a fork candidate, never a benign fresh chain. So a fresh device served a truncated control history detects the gap — per-author chains alone cannot reveal an entirely hidden author (a revocation authored on another device). Two control ops referencing the same predecessor are a detectable fork, resolved by the F14 tie-break (earliest HLC, then lowest member id), the loser's dependents quarantined. This lives in the control-op payload, not the envelope header, so the header layout above is unaffected.

A **MemberRegister** is authored by the enrolling Member itself, as its own first op (`author_seq == 1`, enforced at both ends); Root is not a pseudo-member and never authors an envelope. Root's authority rides in the payload as a **registration certificate** — signed bytes, base64 in the payload, verified over the literal decoded blob and only then parsed. Its content is `{workspace_id, member_id, member_kind, sign_pk, sign_key_id, kex_pk, kex_key_id, registered_at_hlc}`. **The domain string is the version:** the certificate carries no version field, and any field addition or semantic change ships under a new signing domain, so old certificates stay verifiable under the old domain and a downgrade is a signature failure rather than a parsing ambiguity. A verifier checks the envelope signature against the certificate's *own* key: a genuine certificate is public once it is in the log, and without that step anyone holding a copy could wrap it around self-signed envelopes and fork the victim's chain.
...
```
blob = magic "JVE1" 4B || m_cost_kib u32 || t_cost u32 || parallelism u8
     || salt 16B || nonce 24B
     || XChaCha20-Poly1305(Argon2id(passphrase, salt, params), nonce,
            aad = the 53 header bytes above, root_sk 32B || master_wrap_key 32B)

root_sig = Ed25519(root_sk, "jeeves/escrow/v1" || workspace_id (16 raw bytes)
                            || version (u64 big-endian) || blob)
```

The KDF parameters live **inside** the blob and the client enforces the floor on read *and* write, refusing a below-floor blob before any key derivation runs — which is what closes F12's weakened-params-at-write attack, and what lets a future native implementation raise the cost without a protocol change. `workspace_id` sits inside the signed preimage, so a blob signed for one Workspace cannot be replayed into another's slot; the version does too, so a signature does not transfer between versions. The first write establishes `root_pk` in the slot and must be version 1; every later write is checked against the stored `root_pk`, so a stolen *user* credential cannot overwrite the escrow (F16). A device holding a pinned `root_pk` verifies the signature *before* any passphrase prompt: a substituted or rolled-back record is a server-integrity **alarm**, never "wrong passphrase". An enrolling device with no pin unwraps first (TOFU against the passphrase), pins `root_pk` from the recovered `root_sk`, and cross-checks the fetched signature against it.

**Error details on the sync routes** are structured objects — `{"code": <snake_case>, …}`, never a bare string — and a per-op rejection on `POST /w/{w}/ops` additionally carries the zero-based batch `index` of the offending op, so a client branches on which rule fired rather than parsing prose. This governs route error details only; a client's own quarantine vocabulary is a separate namespace.
