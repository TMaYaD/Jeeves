...
* **Data Schema:** Tasks must support the following metadata:
    ...
    * Time Estimate
    * Energy Level
* **System Views:** Auto-generated lists for 'Next Actions', 'Waiting For' (clarified, non-done, `intent = 'next'` todos carrying at least one `Tag(type='person')` via the `todo_tags` junction, grouped by Person), and 'Maybe' (todos with `intent = 'maybe'`).
