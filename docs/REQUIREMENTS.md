...
* **Evening Shutdown:** End-of-day guided ritual covering three steps: (1) review completed tasks against time estimates, (2) resolve each unfinished task (roll over to tomorrow, return to next actions, or defer to Someday/Maybe), (3) view daily summary stats and close the day. The nudge fires only after daily planning has been completed for the day. It is triggered at a user-configured time (default 18:00) via a local notification and a dismissible in-app banner. Both the notification and the banner can be turned off independently in Settings. The ritual can also be launched manually from Settings at any time.
