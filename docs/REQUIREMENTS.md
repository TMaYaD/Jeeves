...
* **Data Schema:** Tasks must support the following metadata:
    ...
    * Time Estimate
    * Energy Level
* **System Views:** Auto-generated lists for 'Next Actions', 'Waiting For' (todos where `waiting_for IS NOT NULL AND clarified = true AND done_at IS NULL AND intent = 'next'`), and 'Maybe' (todos with `intent = 'maybe'`).
...
* **Weekly Review Wizard:** A 6-screen ceremony surfaced when the user has not completed a review in seven days. The wizard runs through four GTD review steps (Process Inbox → Review Waiting For → Review Projects → Review Someday/Maybe), an Objectives step (3–5 goals for the upcoming week, pre-populated from the previous review), and a Summary screen. There is no separate brain-dump/capture step — capture happens via the inbox throughout the week (share-sheet, voice, manual entry), so the review only clarifies what was already captured. The cadence is fixed at 7 days.
    ...
    * **Behaviour:** Each list-driven step iterates through items one at a time using a stable snapshot loaded when the step opens, so the order doesn't shift while the user is working. The footer Next button advances the per-step item cursor first (Back symmetrically retreats it); only when the cursor has no more items does Next cross into the following step. Inbox clarification uses the same card UI as the daily-planning ritual. Steps with empty list snapshots (Inbox, Waiting For) auto-skip on entry; Projects and Someday/Maybe are reflection steps and require a manual Next per item. Tapping Done finalises the completion timestamp (synced across devices) and returns the user to the inbox.
