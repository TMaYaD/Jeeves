...
* **Clarify Mode:** A synced user preference (Settings → Clarify) choosing how a Capture leaves the Inbox. **One Capture, one Outcome** (`oneToOne`, the default) clarifies each Capture to exactly one Outcome, which leaves the Inbox at that first link. **Split and merge** (`nToM`) is selectable but **not yet implemented**: when implemented it will let one Capture carve out several Outcomes and several Captures merge into one, keeping the Capture in the Inbox until the user completes it explicitly. Selecting it today changes nothing — every clarify surface still stamps at the first Outcome link. Both modes run over the same many-to-many Capture↔Outcome storage — the preference governs only when `captures.clarified_at` is stamped, so switching never migrates data. The preference itself persists and replicates across devices.
