...
* **Daily Planning Ritual:** A guided 6-step workflow:
    ...
    4. **Today's Schedule (Step 4):** Review the due dates of the selected tasks — confirm or move each due date (a due-date edit, not a state change; Timebox placement is the planner's job, TMaYaD/Jeeves#40).
    5. **Time Estimates (Step 5):** Set time estimates on any selected task that is still missing one.
    6. **Today's Plan (Step 6):** Summary showing tasks with a due date first (ascending), then the rest. Capacity bar warns if planned time exceeds available time. "Start Day" finalises the plan.
