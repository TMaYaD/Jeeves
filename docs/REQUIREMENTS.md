...
* **Global Capture:** Capture is reachable from every working surface in the app, not only the Inbox — a persistent capture FAB opens a modal capture sheet that writes a raw Capture straight to the Inbox (`clarified_at IS NULL`, `capture_source = 'manual'`), using the same write path as the Inbox quick-add. The FAB is present on every shell list route, task detail, all three ceremony wizards and their completion screens, Search, Active Focus, Clarify, Settings and Import; it is suppressed on the Inbox (quick-add already owns capture there), on the auth screens, and on Evening Shutdown's Close Day. The sheet stays open after each submit — the field clears and refocuses, and an inline confirmation reports the Capture landed — so several thoughts can be offloaded in one visit without leaving the current screen. Clarification remains a separate act; the FAB never creates an Outcome. See [DESIGN.md § Global capture](./DESIGN.md) for the visual and interaction contract.
